...
По умолчанию Gramps использует файловую базу данных SQLite для хранения семейного дерева. Это отлично работает для Gramps Web и рекомендуется для большинства пользователей. Однако, начиная с версии 0.3.0 API Gramps Web, также поддерживается сервер PostgreSQL с одним семейным деревом на базу данных, работающий на основе [дополнения Gramps PostgreSQL](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL). С [версии 1.0.0](https://github.com/gramps-project/gramps-web-api/releases/tag/v1.0.0) также поддерживается дополнение SharedPostgreSQL, которое позволяет размещать несколько семейных деревьев в одной базе данных, что особенно полезно при использовании вместе с поддержкой [многих деревьев API Gramps Web](multi-tree.md).

!!! warning "Поддержка бэкенда PostgreSQL устарела"
    Поддержка бэкенда PostgreSQL (одно семейное дерево на базу данных) будет удалена в будущей версии API Gramps Web, так как она несовместима с размещением нескольких деревьев. Бэкенды SharedPostgreSQL и SQLite остаются полностью поддерживаемыми. Для новых установок используйте SharedPostgreSQL.

## Настройка сервера PostgreSQL

Если вы хотите настроить новую базу данных для использования с дополнением PostgreSQL, вы можете следовать [инструкциям в Wiki Gramps](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL) для настройки сервера.
...
Если вы настроили сервер PostgreSQL самостоятельно, вы можете следовать [инструкциям в Wiki Gramps](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL) для импорта семейного дерева в базу данных.

Кроме того, если вы следовали инструкциям по Docker Compose выше, вы можете использовать следующую команду для импорта файла XML Gramps, расположенного на вашем хосте Docker:
...
## Настройка Web API для использования с базой данных

Чтобы настроить Web API для использования с базой данных PostgreSQL, добавьте следующее под ключом `environment:` сервиса `grampsweb` в `docker-compose.yml`:
...
С версии 2.4.0 API Gramps Web индекс поиска размещается либо в базе данных SQLite (по умолчанию), либо в базе данных PostgreSQL. Также для этой цели можно использовать образ `gramps-postgres`. Для индекса поиска мы можем использовать базу данных `gramps`, предоставленную образом, независимо от того, размещаем ли мы наши генеалогические данные в PostgreSQL или нет (индекс поиска и генеалогические данные могут сосуществовать в одной базе данных). Это можно сделать, в приведенном выше примере, установив опцию конфигурации `SEARCH_INDEX_DB_URI` в
```
...
В случае возникновения проблем, пожалуйста, следите за выводом журнала Gramps Web и сервера PostgreSQL. В случае с Docker это достигается с помощью
