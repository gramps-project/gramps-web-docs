...
Самым удобным вариантом для размещения Gramps Web на вашем собственном сервере (или виртуальном сервере) является использование Docker Compose.
...
Это создаст шесть именованных томов, чтобы убедиться, что все соответствующие данные будут сохранены при перезапуске контейнера.
...
## Шаг 2: Безопасный доступ с помощью SSL/TLS

Веб-API **должен** обслуживаться для публичного интернета через HTTPS. Существует несколько вариантов, например:
...
Смотрите [Docker с Let's Encrypt](lets_encrypt.md) для получения информации о том, как настроить первый вариант.

Если вы планируете использовать Gramps Web только в вашей локальной сети, вы можете пропустить этот шаг.
...
- Создать учетную запись для пользователя-администратора
...
- Возможно, самым удобным вариантом является использование [Gramps Web Sync](../administration/sync.md).
