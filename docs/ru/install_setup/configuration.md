...
Ключ | Описание
----|-------------
`TREE` | Имя базы данных семейного дерева, которую следует использовать. Показать доступные деревья с помощью `gramps -l`. Если дерева с таким именем не существует, будет создано новое пустое.
`SECRET_KEY` | Секретный ключ для flask. Секрет не должен быть публично доступен. Изменение его сделает недействительными все токены доступа.
...
Ключ | Описание
----|-------------
`MEDIA_BASE_DIR` | Путь, который следует использовать в качестве базового каталога для медиафайлов, переопределяющий базовый каталог медиафайлов, установленный в Gramps. При использовании [S3](s3.md) должен иметь вид `s3://<bucket_name>`
 `TREE_ID` | Имя каталога базы данных семейного дерева, которое следует использовать в режиме одного дерева (когда `TREE` не установлено в `MULTI`). При установке сервер идентифицирует дерево по его имени каталога, а не по его отображаемому имени, что более надежно при переименованиях. Обязательно, если вы хотите переименовать дерево через API. Имя каталога можно найти с помощью `GET /api/trees/-` (поле `id`).
...
`STATIC_PATH` | Путь для обслуживания статических файлов (например, статический веб-интерфейс).
`BASE_URL` | Базовый URL, по которому можно получить доступ к API (например, `https://mygramps.mydomain.com/`). Это необходимо, например, для построения корректных ссылок для сброса пароля.
`CORS_ORIGINS` | Происхождения, откуда разрешены CORS-запросы. По умолчанию все запрещены. Используйте `"*"` для разрешения запросов с любого домена.
...
`PILLOW_MAX_IMAGE_PIXELS` | Устанавливает параметр PIL.Image.MAX_IMAGE_PIXELS, который указывает количество пикселей, которое может содержать обрабатываемое изображение. См. [документацию](https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.MAX_IMAGE_PIXELS) для получения подробной информации.
`MAX_THUMBNAIL_FILE_BYTES` | Устанавливает жесткий максимальный размер файла для миниатюр. По умолчанию `50 * 1024 * 1024` (50 МБ). Увеличение этого значения может значительно увеличить использование памяти и привести к сбоям из-за недостатка памяти или потере данных, если большие файлы распаковываются в памяти.
