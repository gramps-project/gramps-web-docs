...
Щоб увімкнути аутентифікацію OIDC, вам потрібно налаштувати відповідні параметри у файлі конфігурації Gramps Web або змінних середовища. Дивіться сторінку [Конфігурація сервера](configuration.md#settings-for-oidc-authentication) для повного списку доступних налаштувань OIDC.

!!! info
    Коли ви використовуєте змінні середовища, пам'ятайте, що потрібно додати префікс до кожного імені налаштування `GRAMPSWEB_` (наприклад, `GRAMPSWEB_OIDC_ENABLED`). Дивіться [Файл конфігурації vs. змінні середовища](configuration.md#configuration-file-vs-environment-variables) для деталей.
...
Посібник з налаштування OIDC для Gramps Web, створений спільнотою, доступний на [офіційному веб-сайті документації Authelia](https://www.authelia.com/integration/openid-connect/clients/gramps/).
