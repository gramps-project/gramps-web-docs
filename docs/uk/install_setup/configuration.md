...
Ключ | Опис
----|-------------
`MEDIA_BASE_DIR` | Шлях, який використовуватиметься як базовий каталог для медіафайлів, перекриваючи базовий каталог медіа, встановлений у Gramps. При використанні [S3](s3.md) повинен мати форму `s3://<bucket_name>`
 `TREE_ID` | Назва каталогу бази даних родинного дерева, яку потрібно використовувати в режимі одного дерева (коли `TREE` не встановлено на `MULTI`). Коли встановлено, сервер ідентифікує дерево за його назвою каталогу, а не за його відображуваною назвою, що є більш надійним при перейменуваннях. Обов'язково, якщо ви хочете перейменувати дерево через API. Назву каталогу можна знайти за допомогою `GET /api/trees/-` (поле `id`).
...
`STATIC_PATH` | Шлях для обслуговування статичних файлів (наприклад, статичного веб-фронтенду).
...
`CORS_ORIGINS` | Джерела, з яких дозволені запити CORS. За замовчуванням усі заборонені. Використовуйте `"*"` для дозволу запитів з будь-якого домену.
...
`OIDC_ISSUER` | URL постачальника OIDC (для нестандартних постачальників OIDC).
`OIDC_CLIENT_ID` | OAuth ідентифікатор клієнта (для нестандартних постачальників OIDC).
`OIDC_CLIENT_SECRET` | OAuth секрет клієнта (для нестандартних постачальників OIDC).
`OIDC_NAME` | Нестандартна відображувана назва для постачальника. За замовчуванням "OIDC".
...
`OIDC_GROUP_ADMIN` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Admin" в Gramps.
`OIDC_GROUP_OWNER` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Owner" в Gramps.
`OIDC_GROUP_EDITOR` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Editor" в Gramps.
`OIDC_GROUP_CONTRIBUTOR` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Contributor" в Gramps.
`OIDC_GROUP_MEMBER` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Member" в Gramps.
`OIDC_GROUP_GUEST` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Guest" в Gramps.
...
`LLM_MODEL` | Модель, яку потрібно використовувати для API чату, сумісного з OpenAI. Якщо не встановлено (за замовчуванням), чат вимкнено. Починаючи з v3.6.0, асистент ШІ використовує Pydantic AI з можливістю виклику інструментів.
`VECTOR_EMBEDDING_MODEL` | Модель [Sentence Transformers](https://sbert.net/) для семантичних векторних вкладів пошуку. Якщо не встановлено (за замовчуванням), семантичний пошук і чат вимкнені.
`LLM_MAX_CONTEXT_LENGTH` | Обмеження символів для контексту родинного дерева, наданого LLM. За замовчуванням 50000.
`LLM_SYSTEM_PROMPT` | Нестандартний системний запит для асистента чату LLM (v3.6.0+). Якщо не встановлено, використовується стандартний запит, оптимізований для генеалогії.
