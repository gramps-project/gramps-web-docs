...
Використовуючи стандартний образ Docker, всі необхідні налаштування можна зробити з браузера. Проте, в залежності від розгортання, може бути необхідно налаштувати конфігурацію сервера.
...
Зверніть увагу, що параметри конфігурації, встановлені через середовище, мають вищий пріоритет над тими, що в файлі конфігурації. Якщо обидва присутні, змінна середовища "перемагає".
...
`REGISTRATION_DISABLED` | Якщо `True`, заборонити нову реєстрацію користувачів (за замовчуванням `False`).
...
Ключ | Опис
----|-------------
`OIDC_ENABLED` | Булеве значення, чи увімкнути аутентифікацію OIDC. За замовчуванням `False`.
`OIDC_ISSUER` | URL постачальника OIDC (для користувацьких постачальників OIDC).
`OIDC_CLIENT_ID` | OAuth ID клієнта (для користувацьких постачальників OIDC).
...
`VECTOR_EMBEDDING_MODEL` | Модель [Sentence Transformers](https://sbert.net/) для семантичних пошукових векторних вкладів. Якщо не встановлено (за замовчуванням), семантичний пошук і чат вимкнені.
...
EMAIL_PORT=465
EMAIL_USE_SSL=True  # Використовувати неявний SSL для порту 465
