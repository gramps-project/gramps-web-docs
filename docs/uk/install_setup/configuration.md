...
Ця сторінка містить усі методи зміни конфігурації та всі наявні параметри конфігурації.
...
Коли ви використовуєте [налаштування на основі Docker Compose](deployment.md), ви можете включити файл конфігурації, додавши наступний елемент списку під ключем `volumes:` в блоці `grampsweb:`:
...
`SEARCH_INDEX_DB_URI` | URL бази даних для індексу пошуку. Дозволені лише `sqlite` або `postgresql` як бекенди. За замовчуванням `sqlite:///indexdir/search_index.db`, створюючи файл SQLite в папці `indexdir`, відносно шляху, де запускається скрипт.
...
`EMAIL_USE_TLS` | **Застаріло** (використовуйте `EMAIL_USE_SSL` або `EMAIL_USE_STARTTLS` замість). Булеве значення, чи використовувати TLS для надсилання електронних листів. За замовчуванням `True`. При використанні STARTTLS, встановіть це значення на `False` і використовуйте порт, відмінний від 25.
...
`MAX_THUMBNAIL_FILE_BYTES` | Встановлює жорсткий максимальний розмір файлу для мініатюр. За замовчуванням `50 * 1024 * 1024` (50 МБ). Підвищення цього значення може значно збільшити використання пам'яті і може призвести до аварійних зупинок через нестачу пам'яті або втрати даних, якщо великі файли розпаковуються в пам'яті.
...
`NEW_DB_BACKEND` | Бекенд бази даних, який використовуватиметься для новостворених родинних дерев. Повинен бути одним з `sqlite`, `postgresql` або `sharedpostgresql`. За замовчуванням `sqlite`.
...
`OIDC_CLIENT_ID` | OAuth ідентифікатор клієнта (для кастомних постачальників OIDC)
`OIDC_CLIENT_SECRET` | OAuth секрет клієнта (для кастомних постачальників OIDC)
...
`LLM_MODEL` | Модель, яку потрібно використовувати для API чату, сумісного з OpenAI. Якщо не встановлено (за замовчуванням), чат вимкнено. Починаючи з версії v3.6.0, асистент ШІ використовує Pydantic AI з можливостями виклику інструментів.
`VECTOR_EMBEDDING_MODEL` | Модель [Sentence Transformers](https://sbert.net/) для семантичних векторних вбудовувань пошуку. Якщо не встановлено (за замовчуванням), семантичний пошук і чат вимкнено.
`LLM_MAX_CONTEXT_LENGTH` | Обмеження символів для контексту родинного дерева, наданого LLM. За замовчуванням 50000.
`LLM_SYSTEM_PROMPT` | Кастомний системний запит для асистента чату LLM (v3.6.0+). Якщо не встановлено, використовується стандартний запит, оптимізований для генеалогії.
