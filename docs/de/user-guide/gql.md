...
Die [Objektlistenansichten](lists.md) (Personen, Familien, Ereignisse, ...) haben einen optionalen erweiterten Filtermodus, der auf der [Gramps Abfragesprache](https://github.com/DavidMStraub/gramps-ql) (GQL) basiert.
...
Eine GQL-Abfrage ist eine Zeichenkette, die aus Aussagen der Form `property operator value` besteht, die optional mit den Schlüsselwörtern `and` und `or` sowie Klammern kombiniert werden kann.
...
GQL unterstützt das Abfragen von verschachtelten Eigenschaften von Gramps-Objekten, z.B. `primary_name.date.calendar`. Siehe unten für eine vollständige Liste der Eigenschaften – siehe auch [Gramps-Datenmodell](https://gramps-project.org/wiki/index.php/Gramps_Data_Model).
...
Zwei weitere spezielle Eigenschaften für array-ähnliche Gramps-Eigenschaften. `all` erfordert, dass eine Bedingung für alle Elemente der Liste gilt, `any` erfordert, dass sie für mindestens ein Element gilt. Beide Eigenschaften können mit anderen Eigenschaften davor und danach kombiniert werden. Beispiele: `media_list.any.citation_list.length > 0`, um Objekte mit Medienreferenzen zurückzugeben, die Zitationen haben; `media_list.all.citation_list.length = 0`, um Objekte zurückzugeben, bei denen alle Medienobjekte keine Zitationen haben.
...
Enthält oder enthält nicht. Funktioniert für Listen sowie Strings. Beispiele: `gramps_id !~ F00`, `author ~ David`, `family_list ~ "3a16680f7d226e3ac3eefc8b57a"`

#### Kein Operator/Wert

Wenn kein Operator und kein Wert angegeben ist, wird der Wert als boolesch (true oder false) interpretiert. Dies funktioniert für alle Arten von Eigenschaften, und die Python-Regeln für das Casting zu true/false werden angewendet. Zum Beispiel gibt die Abfrage `private` private Objekte zurück; `confidence` gibt Objekte zurück, bei denen die Zuversicht größer als 0 ist; `media_list` gibt Objekte mit mindestens einer Medienreferenz zurück.
...
Alle Objekte, die *keine* Person sind, aber eine Medienreferenz haben, die Teil eines Bildes ist. Hier bedeutet `media_list.any.rect`, dass für jedes der Elemente in der Medienliste überprüft wird, ob die `rect` (Rechteck)-Eigenschaft einen wahrheitswertigen Wert hat, was bedeutet, dass es sich um eine nicht leere Liste handelt. (`media_list.any.rect.length > 0` hätte den gleichen Effekt.)
...
Ereignisse, bei denen das Datum ein normales Datum ist (nicht ein Bereich usw.) und das Jahr nach 2020 liegt.

```sql
note_list.any.get_note.text.string ~ "David"
```

Alle Objekte mit mindestens einer Notiz, die den String "David" in ihrem Text enthalten.
