...
- Verwenden Sie doppelte Unterstriche für geschachtelte Dictionary-Einstellungen, z. B. wird `GRAMPSWEB_THUMBNAIL_CACHE_CONFIG__CACHE_DEFAULT_TIMEOUT` den Wert der Konfigurationsoption `THUMBNAIL_CACHE_CONFIG['CACHE_DEFAULT_TIMEOUT']` setzen
...
`TREE_ID` | Der Verzeichnisname der Familienstammbaum-Datenbank, die im Einzelbaum-Modus verwendet werden soll (wenn `TREE` nicht auf `*` gesetzt ist). Wenn festgelegt, identifiziert der Server den Baum anhand seines Verzeichnisnamens anstelle seines Anzeigenamens, was robuster gegenüber Umbenennungen ist. Erforderlich, wenn Sie den Baum über die API umbenennen möchten. Der Verzeichnisname kann über `GET /api/trees/-` (das Feld `id`) gefunden werden.
...
`SEARCH_INDEX_DIR` | **Veraltet** (verwenden Sie stattdessen `SEARCH_INDEX_DB_URI`). Verzeichnis, das den Suchindex enthält. Wenn festgelegt, während `SEARCH_INDEX_DB_URI` nicht gesetzt ist, wird die Suchindex-URL als `sqlite:///<SEARCH_INDEX_DIR>/search_index.db` abgeleitet.
...
`EMAIL_USE_SSL` | Boolean, ob implizites SSL/TLS für SMTP verwendet werden soll (v3.6.0+). Standardmäßig `True`, wenn `EMAIL_USE_TLS` nicht ausdrücklich festgelegt ist. Wird typischerweise mit Port 465 verwendet.
...
`DISABLE_TELEMETRY` | Wenn `True`, wird die Telemetriedatenstatistik deaktiviert (Standard `False`). Siehe [Telemetrie](telemetry.md) für Details.
...
Schlüssel | Beschreibung
----|-------------
`MEDIA_PREFIX_TREE` | Boolean, ob ein separates Unterverzeichnis für die Mediendateien jedes Baumes verwendet werden soll oder nicht. Standardmäßig `False`, aber es wird dringend empfohlen, `True` in einem Multi-Baum-Setup zu verwenden.
...
Diese Einstellungen ermöglichen es Ihnen, OIDC-Gruppen/Rollen von Ihrem Identitätsanbieter den Gramps-Webbenutzerrollen zuzuordnen:
...
`LLM_SYSTEM_PROMPT` | Benutzerdefinierter System-Prompt für den LLM-Chat-Assistenten (v3.6.0+). Wenn nicht festgelegt, wird der standardmäßige genealogie-optimierte Prompt verwendet.
