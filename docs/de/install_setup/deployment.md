...
!!! warning
    Obiges macht die API auf Port 80 der Host-Maschine **ohne SSL/TLS-Schutz** verfügbar. Sie können dies für lokale Tests verwenden, aber setzen Sie dies nicht direkt dem Internet aus, es ist völlig unsicher!

## Schritt 2: Sicherer Zugriff mit SSL/TLS
...
Wenn Sie Gramps Web nur in Ihrem lokalen Netzwerk verwenden möchten, können Sie diesen Schritt überspringen.
...
Beim ersten Ausführen zeigt die App einen Ersteinrichtungsassistenten an, der es Ihnen ermöglicht,

- ein Konto für den Eigentümer (Admin)-Benutzer zu erstellen
- einige notwendige Konfigurationsoptionen festzulegen
- einen Stammbaum im Gramps XML (`.gramps`)-Format zu importieren
...
- Die arguably bequemste Option ist die Verwendung von [Gramps Web Sync](../administration/sync.md).
