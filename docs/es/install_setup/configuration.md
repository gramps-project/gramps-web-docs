...
Cuando utilizas la [configuración basada en Docker Compose](deployment.md), puedes incluir un archivo de configuración agregando el siguiente elemento a la lista bajo la clave `volumes:` en el bloque `grampsweb:`:
...
Clave | Descripción
----|-------------
`TREE` | El nombre de la base de datos del árbol genealógico a utilizar. Muestra los árboles disponibles con `gramps -l`. Si un árbol con este nombre no existe, se creará uno nuevo vacío.
...
`CORS_ORIGINS` | Orígenes desde donde se permiten solicitudes CORS. Por defecto, todos están deshabilitados. Usa `"*"` para permitir solicitudes desde cualquier dominio.
...
!!! info
    Al usar variables de entorno para la configuración, las opciones booleanas como `EMAIL_USE_TLS` deben ser ya sea la cadena `true` o `false` (¡sensible a mayúsculas!).
...
`VECTOR_EMBEDDING_MODEL` | El modelo de [Sentence Transformers](https://sbert.net/) a utilizar para las incrustaciones de vectores de búsqueda semántica. Si no se establece (el valor predeterminado), la búsqueda semántica y el chat están deshabilitados.
...
`LLM_SYSTEM_PROMPT` | Indicación del sistema personalizada para el asistente de chat LLM (v3.6.0+). Si no se establece, utiliza la indicación optimizada para genealogía por defecto.
