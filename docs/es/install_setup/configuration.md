...
Cuando usas la [configuración basada en Docker Compose](deployment.md), puedes incluir un archivo de configuración añadiendo el siguiente elemento a la lista bajo la clave `volumes:` en el bloque `grampsweb:`:
...
`PERSISTENT_CACHE_CONFIG` | Diccionario con ajustes para la caché persistente, utilizada, por ejemplo, para telemetría. Consulta [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) para posibles configuraciones.
...
`LLM_MODEL` | El modelo a utilizar para la API de chat compatible con OpenAI. Si no se establece (el valor por defecto), el chat está deshabilitado. A partir de v3.6.0, el asistente de IA utiliza Pydantic AI con capacidades de llamada a herramientas.
