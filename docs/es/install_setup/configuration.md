...
Cuando uses la [configuración basada en Docker Compose](deployment.md), puedes incluir un archivo de configuración agregando el siguiente elemento de lista bajo la clave `volumes:` en el bloque `grampsweb:`:
...
!!! warning "Las variables de entorno sin prefijo están en desuso"
    Por razones históricas, un puñado de configuraciones – `TREE`, `SECRET_KEY`, `USER_DB_URI`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `MEDIA_BASE_DIR`, `SEARCH_INDEX_DIR`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `DEFAULT_FROM_EMAIL`, `BASE_URL`, y `STATIC_PATH` – aún se pueden establecer a través de una variable de entorno *sin* el prefijo `GRAMPSWEB_`. Esto está en desuso, genera una advertencia al iniciar y dejará de funcionar en una futura versión. Siempre usa la forma con prefijo, por ejemplo, `GRAMPSWEB_TREE` en lugar de `TREE`.

    Ten en cuenta que esto solo concierne a las variables de entorno. En un archivo de configuración, los nombres de configuración siempre se utilizan sin prefijo.

!!! tip "Verificando opciones en desuso"
    Las opciones de configuración en desuso de las que tu servidor aún depende – como las variables de entorno sin prefijo, `SEARCH_INDEX_DIR`, o `EMAIL_USE_TLS` – se registran como advertencias al iniciar. Desde Gramps Web API 3.22, también se enumeran, junto con su reemplazo y la versión en la que se eliminará el soporte, en la parte superior de la página **Información del Sistema** (accesible a través del icono de usuario en la barra de aplicaciones superior) cuando estás conectado como administrador.
...
`SEARCH_INDEX_DB_URI` | URL de la base de datos para el índice de búsqueda. Solo se permiten `sqlite` o `postgresql` como backends. Por defecto, se establece en `sqlite:///indexdir/search_index.db`, creando un archivo SQLite en la carpeta `indexdir` relativa a la ruta donde se ejecuta el script.
`SEARCH_INDEX_DIR` | **En desuso** (usa `SEARCH_INDEX_DB_URI` en su lugar). Directorio que contiene el índice de búsqueda. Si se establece mientras `SEARCH_INDEX_DB_URI` está sin definir, la URL del índice de búsqueda se deriva como `sqlite:///<SEARCH_INDEX_DIR>/search_index.db`.
...
!!! info
    Al usar variables de entorno para la configuración, las opciones booleanas como `EMAIL_USE_SSL` deben ser ya sea la cadena `true` o `false` (¡sensible a mayúsculas!).
...
`NEW_DB_BACKEND` | El backend de la base de datos a utilizar para los árboles genealógicos recién creados. Debe ser uno de `sqlite`, `postgresql`, o `sharedpostgresql`. Por defecto es `sqlite`.
...
`VECTOR_EMBEDDING_MODEL` | El modelo a utilizar para las incrustaciones de vectores de búsqueda semántica. Al usar un modelo local, este debe ser un nombre de modelo de [Sentence Transformers](https://sbert.net/). Al usar una API remota (ver `VECTOR_EMBEDDING_BASE_URL`), este es el nombre del modelo pasado al proveedor remoto. Si no se establece (el valor predeterminado), la búsqueda semántica y el chat están deshabilitados.
`VECTOR_EMBEDDING_BASE_URL` | URL base para una API de incrustación compatible con OpenAI remota (por ejemplo, Ollama, OpenAI, LiteLLM). Si no se establece (el valor predeterminado), se utiliza un modelo local de Sentence Transformers. Consulta [Uso de una API de incrustación remota](chat.md#using-a-remote-embedding-api) para más detalles.
`VECTOR_EMBEDDING_API_KEY` | Clave API para proveedores de incrustación remota autenticados. Solo es necesaria cuando `VECTOR_EMBEDDING_BASE_URL` está establecido y el proveedor requiere autenticación.
...
`LLM_SYSTEM_PROMPT` | Aviso del sistema personalizado para el asistente de chat LLM (v3.6.0+). Si no se establece, utiliza el aviso predeterminado optimizado para genealogía.
