...
Asumiremos que Docker y Docker Compose ya están instalados en tu sistema. Puedes usar Windows, Mac OS o Linux como sistema host. Las arquitecturas soportadas incluyen no solo x86-64 (sistemas de escritorio), sino también sistemas ARM como una Raspberry Pi, que puede servir como un servidor web de bajo costo, pero potente (suficiente).
...
!!! warning
    Lo anterior hará que la API esté disponible en el puerto 80 de la máquina host **sin protección SSL/TLS**. Puedes usar esto para pruebas locales, pero no lo expongas directamente a Internet, ¡es completamente inseguro!
