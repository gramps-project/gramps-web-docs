...
Esto hará que todos los árboles en el directorio de la base de datos de Gramps del servidor sean accesibles (dadas las suficientes permisos de usuario). La ID del árbol es el nombre del subdirectorio. Puedes listar los árboles existentes (nombres e IDs) con el comando
...
La página de registro accesible desde la página de inicio de sesión no funciona en una configuración de múltiples árboles, ya que se necesita especificar un árbol para el registro. Por lo tanto, es aconsejable establecer `hideRegisterLink` en `true` en la [configuración del frontend](frontend-config.md).
