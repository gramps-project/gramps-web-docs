# Filtro usando a Linguagem de Consulta Gramps

As [visualizações de lista de objetos](lists.md) (pessoas, famílias, eventos, ...) têm um modo de filtro avançado opcional baseado na [Linguagem de Consulta Gramps](https://github.com/DavidMStraub/gramps-ql) (GQL).
...
Filtra pela classe de objeto Gramps e pode ser uma de `person`, `family`, `event`, `place`, `citation`, `source`, `repository`, `media` ou `note`.

#### Propriedades do objeto

O GQL suporta consultas em propriedades aninhadas de objetos Gramps, por exemplo, `primary_name.date.calendar`. Veja abaixo uma lista completa de propriedades – veja também [Modelo de Dados Gramps](https://gramps-project.org/wiki/index.php/Gramps_Data_Model).
...
Enquanto todas as propriedades anteriores se referem a um único objeto Gramps, também é possível filtrar em diferentes objetos referidos pelo objeto inicial. Por exemplo, um evento tem um identificador de lugar em sua propriedade `place`. Usando a pseudo-propriedade `get_place`, o GQL muda para as propriedades desse objeto. Por exemplo, é possível pesquisar `class = event and place.get_place.name.value ~ York`. Isso também pode ser combinado com `any` ou `all`, por exemplo, `class = person and event_ref_list.any.ref.get_event.description ~ farmer`.
...
Comparação. Funciona para strings e números. Exemplos: `confidence <= 1`, `change > 1712477760 `, `gramps_id > "I2015"`

#### `~`, `!~`

Contém ou não contém. Funciona para listas e strings. Exemplos: `gramps_id !~ F00`, `author ~ David`, `family_list ~ "3a16680f7d226e3ac3eefc8b57a"`

#### Sem operador/valor

Se nenhum operador e valor forem dados, o valor é interpretado como um booleano (true ou false). Isso funciona para todos os tipos de propriedades e as regras do Python para conversão para true/false são aplicadas. Por exemplo, a consulta `private` retorna objetos privados; `confidence` retorna objetos onde a confiança é maior que 0; `media_list` retorna objetos com pelo menos uma referência de mídia.

### Valores

Os valores podem ser números ou strings. Se os números devem ser interpretados como strings ou caracteres especiais como = estão envolvidos, coloque o valor entre aspas. Exemplos: `gramps_id = F0001`, mas `gramps_id = "0001"`.
...
## Lista completa de Propriedades Gramps

Para uma lista completa de propriedades Gramps, veja a [documentação GQL](https://github.com/DavidMStraub/gramps-ql#full-list-of-gramps-properties).
