# Use o gerenciamento de tarefas embutido

O Gramps Web contém uma ferramenta de gerenciamento de tarefas genealógicas embutida. Ela é destinada a permitir que os pesquisadores planejem e priorizem, mas também documentem suas tarefas. É por isso que as tarefas são representadas como fontes no banco de dados do Gramps. Após concluir uma tarefa, o conteúdo associado pode servir como uma fonte documentando o processo de pesquisa.
...
- Tags. Os rótulos são tags normais do Gramps da fonte subjacente. (Observe que todas as tarefas têm adicionalmente o rótulo `ToDo` para identificá-las como tarefas, mas esse rótulo é ocultado na lista de tarefas para evitar desordem.)
...
Para editar qualquer detalhe da tarefa, clique sobre ele na lista de tarefas.

A página de detalhes da tarefa não possui um "modo de edição" separado como outros objetos do Gramps. As alterações no título, status e prioridade são aplicadas imediatamente. Alterações na descrição em texto rico exigem que você clique no botão "salvar" abaixo dela.
