...
`tree` é necessário em configurações multi-árvore; omiti-lo ou passar o ID de uma árvore que não existe falha no login. Em um servidor de árvore única, `tree` é opcional, mas se fornecido, deve corresponder ao `TREE` configurado.
...
## URIs de Redirecionamento Requeridas
...
Chave | Descrição
----|-------------
`OIDC_ROLE_CLAIM` | O nome da reivindicação no token OIDC que contém os grupos/papéis do usuário. Padrão é "groups". Caminhos com pontos são suportados, por exemplo, `realm_access.roles`.
...
- Se a reivindicação de papel estiver ausente do token completamente, o papel existente permanece inalterado; uma nova conta ainda é criada com o padrão desativado.
...
Contas criadas através do OIDC recebem um nome de usuário gerado, atribuído uma vez na criação da conta e nunca mudado em logins posteriores:
