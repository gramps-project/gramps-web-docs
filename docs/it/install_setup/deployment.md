# Distribuzione di Gramps Web con Docker

L'opzione più conveniente per ospitare Gramps Web sul tuo server (o server virtuale) è con Docker Compose.
...
Questo genererà sei volumi nominati per garantire che tutti i dati rilevanti persistano al riavvio del contenitore.
...
Consulta [Docker con Let's Encrypt](lets_encrypt.md) per come impostare la prima opzione.
...
Alla prima esecuzione, l'app mostrerà una procedura guidata di primo avvio che ti permetterà di
...
- Quando si utilizzano file memorizzati sullo stesso server di Gramps Web, puoi montare una directory nel contenitore Docker invece di utilizzare un volume nominato, ad esempio `/home/server_user/gramps_media/:/app/media` invece di `gramps_media:/app/media`, e caricare i tuoi file multimediali lì.
- Quando si utilizzano file multimediali [ospitati su S3](s3.md), puoi utilizzare il componente aggiuntivo S3 Media Uploader.
