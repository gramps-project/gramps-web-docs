...
Le [visualizzazioni dell'elenco degli oggetti](lists.md) (persone, famiglie, eventi, ...) hanno una modalità di filtro avanzata opzionale basata sul [Linguaggio di Query di Gramps](https://github.com/DavidMStraub/gramps-ql) (GQL).
...
Una query GQL è una stringa composta da dichiarazioni della forma `proprietà operatore valore`, combinata facoltativamente con le parole chiave `and` e `or` e con le parentesi.

### Proprietà

#### `class`

Filtra per la classe dell'oggetto Gramps e può essere uno tra `person`, `family`, `event`, `place`, `citation`, `source`, `repository`, `media`, o `note`.

#### Proprietà dell'oggetto

GQL supporta la query di proprietà annidate degli oggetti Gramps, ad esempio `primary_name.date.calendar`. Vedi di seguito per un elenco completo delle proprietà – vedi anche [Modello di Dati di Gramps](https://gramps-project.org/wiki/index.php/Gramps_Data_Model).

#### Elementi dell'elenco per indice

Gli elementi individuali nelle proprietà simili a liste possono essere accessibili tramite indice posizionale tra parentesi quadre. Questo può essere combinato con proprietà annidate, ad esempio `primary_name.surname_list[0].surname`.
...
Confronto. Funziona per stringhe così come per numeri. Esempi: `confidence <= 1`, `change > 1712477760 `, `gramps_id > "I2015"`

#### `~`, `!~`

Contiene o non contiene. Funziona per liste così come per stringhe. Esempi: `gramps_id !~ F00`, `author ~ David`, `family_list ~ "3a16680f7d226e3ac3eefc8b57a"`
...
tutti i tipi di proprietà e le regole di Python per il casting a true/false vengono applicate. Ad esempio, la query `private` restituisce oggetti privati; `confidence` restituisce oggetti in cui la fiducia è maggiore di 0; `media_list` restituisce oggetti con almeno un riferimento ai media.
...
Tutti gli oggetti che *non* sono una persona ma hanno un riferimento ai media che è parte di un'immagine. Qui, `media_list.any.rect` significa che per ciascuno degli elementi nell'elenco dei media, viene verificato se la proprietà `rect` (rettangolo) ha un valore veritiero, il che significa che è un elenco non vuoto. (`media_list.any.rect.length > 0` avrebbe lo stesso effetto.)
...
Eventi in cui la data è una data normale (non un intervallo ecc.) e l'anno è successivo al 2020.
