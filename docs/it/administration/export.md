...
Nota che se il tuo utente Gramps Web non ha il permesso di visualizzare record privati, l'esportazione non sarà un backup completo, poiché non conterrà alcun record privato.

Questo file Gramps XML può essere utilizzato in seguito per ripristinare l'albero a questo stato esatto tramite [Ripristina da Backup](settings.md#restore-from-backup).

## Condividi il tuo albero genealogico con utenti di altri programmi genealogici

Quando la condivisione di dati genealogici come Gramps XML non è un'opzione, puoi anche esportare un file GEDCOM. Nota che questo non è adatto come backup del tuo albero Gramps Web.
...
Nota che, specialmente per alberi grandi, questa può essere un'operazione costosa per il server e dovrebbe essere eseguita solo se assolutamente necessario.
...
In entrambi i casi, se il tuo utente Gramps Web non ha il permesso di visualizzare record privati, l'esportazione non conterrà file di oggetti multimediali privati.
...
2. Nella pagina di esportazione, genera e scarica un archivio multimediale. Se utilizzi il [complemento Sync](sync.md), puoi anche semplicemente ZIP il tuo folder multimediale locale di Gramps.
...
Nota che, mentre i tuoi account utente saranno migrati, tutti i tuoi utenti dovranno impostare nuove password utilizzando il link "password dimenticata", poiché le password sono memorizzate in forma crittografata e non possono essere esportate.
