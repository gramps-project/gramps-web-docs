...
- Il numero totale di oggetti multimediali nell'albero e se ce ne sono alcuni privi di checksum
...
Consente di caricare un file ZIP di file multimediali per completare i file mancanti. Vedi [Importa dati](import.md) per istruzioni complete.

## Indice di ricerca

### Gestisci indice di ricerca

Gramps Web mantiene un indice di ricerca a testo completo che di solito viene aggiornato automaticamente ogni volta che i dati cambiano. L'indicatore di stato mostra quanti oggetti sono attualmente indicizzati rispetto al conteggio totale degli oggetti.

Clicca su **Aggiorna indice di ricerca** per attivare una ricostruzione completa. Viene mostrato un indicatore di progresso mentre il compito viene eseguito in background. Questo è di solito necessario solo dopo un aggiornamento del server.
...
Imposta un **colore primario** e un **colore secondario** personalizzati per l'interfaccia di Gramps Web. Questi colori vengono applicati a tutti gli utenti di questo albero e hanno effetto immediato dopo il salvataggio.
...
Usa il selettore di oggetti per cercare e scegliere una nota, quindi salva. Clicca su **Rimuovi** per cancellare la nota corrente della pagina principale.
...
Usa il selettore di oggetti per cercare e scegliere un oggetto multimediale, quindi salva. Clicca su **Rimuovi** per cancellare l'immagine corrente della pagina principale.

### Impostazioni di esportazione/importazione

Le impostazioni a livello di albero (titolo dell'app, colori del tema, nota/imagine della pagina principale, ecc.) possono essere esportate come file JSON per backup o per copiare in un'altra istanza di Gramps Web.
