...
Ancak, Gramps Web API arka uçunun 0.7.0 sürümünden itibaren, tek bir kurulumdan birden fazla ağacı sunmak da mümkündür. Ancak, her kullanıcı (şu anda) tek bir ağaca bağlıdır, bu nedenle bu kurulum, ağaçları kullanıcılar arasında paylaşmak için değil, birden fazla izole Gramps Web örneği barındırmak için uygundur.

## Çoklu Ağaç Desteğini Etkinleştirme
...
Ayrıca, medya dosyalarının ayrı alt klasörlerde saklanmasını sağlamak için `MEDIA_PREFIX_TREE` yapılandırma seçeneğini `True` olarak ayarlamalısınız. Aksi takdirde, kullanıcılar, izinleri olmayan bir ağaca ait medya dosyalarına erişebilir!

## Belirli Bir Ağaca Kullanıcı Hesabı Ekleme
...
Kullanıcı adlarının *tüm* ağaçlar arasında benzersiz olması gerekmektedir. E-posta adreslerinin benzersiz olması gerekmez (Gramps Web API 3.22'den itibaren), bu nedenle aynı kişi, örneğin, tek bir e-posta adresi kullanarak birkaç ağaçta hesaplara sahip olabilir.

## Yeni Bir Ağaç Oluşturma

Yeni bir ağaç oluşturmak için, Gramps CLI kullanmak yerine `/trees/` uç noktasına POST yapmanız önerilir. Bu, ağaç kimliği olarak bir UUIDv4 kullanır; bu da ismin tahmin edilemeyeceği için ek güvenlik sağlar. Şu anda, yalnızca SQLite yeni oluşturulan ağaçlar için desteklenmektedir.
...
## Ön Yüzü Özelleştirme

Giriş sayfasından erişilebilen kayıt sayfası, çoklu ağaç kurulumunda çalışmaz; çünkü kayıt için bir ağacın belirtilmesi gerekir. Bu nedenle, [ön yüz yapılandırmasında](frontend-config.md) `hideRegisterLink` değerini `true` olarak ayarlamak tavsiye edilir.
