...
`MEDIA_BASE_DIR` | Medya dosyaları için temel dizin olarak kullanılacak yol, Gramps'ta ayarlanan medya temel dizinini geçersiz kılar. [S3](s3.md) kullanırken, `s3://<bucket_name>` biçiminde olmalıdır.
 `TREE_ID` | Tek ağaç modunda kullanılacak aile ağacı veritabanının dizin adı (eğer `TREE` `MULTI` olarak ayarlanmamışsa). Ayarlandığında, sunucu ağacı dizin adıyla tanımlar, bu da yeniden adlandırmalara karşı daha dayanıklıdır. API aracılığıyla ağacı yeniden adlandırmak istiyorsanız gereklidir. Dizin adı `GET /api/trees/-` ile bulunabilir ( `id` alanı).
`SEARCH_INDEX_DB_URI` | Arama dizini için veritabanı URL'si. Sadece `sqlite` veya `postgresql` arka uçları kabul edilir. Varsayılan olarak `sqlite:///indexdir/search_index.db`'dir ve bu, scriptin çalıştırıldığı yola göre `indexdir` klasöründe bir SQLite dosyası oluşturur.
...
`REGISTRATION_DISABLED` | Eğer `True` ise, yeni kullanıcı kaydını engelleyin (varsayılan `False`).
`DISABLE_TELEMETRY` | Eğer `True` ise, istatistik telemetrisi devre dışı bırakılır (varsayılan `False`). Ayrıntılar için [telemetri](telemetry.md) sayfasına bakın.
...
`MAX_THUMBNAIL_FILE_BYTES` | Küçük resimler için zorunlu maksimum dosya boyutunu ayarlar. Varsayılan olarak `50 * 1024 * 1024` (50 MB) 'dir. Bunu artırmak, bellek kullanımını büyük ölçüde artırabilir ve büyük dosyalar bellekte açıldığında bellek yetersizliği çökmesine veya veri kaybına yol açabilir.
...
Aşağıdaki ayarlar, [birden fazla ağaç barındırırken](multi-tree.md) geçerlidir.
...
Anahtar | Açıklama
----|-------------
`LLM_BASE_URL` | OpenAI uyumlu sohbet API'si için temel URL. Varsayılan olarak `None`'dur, bu OpenAI API'sini kullanır.
`LLM_MODEL` | OpenAI uyumlu sohbet API'si için kullanılacak model. Ayarlanmadığında (varsayılan), sohbet devre dışıdır. v3.6.0 itibarıyla, AI asistanı araç çağırma yetenekleri ile Pydantic AI kullanır.
...
EMAIL_PORT=465
EMAIL_USE_SSL=True  # Port 465 için örtük SSL kullanın
