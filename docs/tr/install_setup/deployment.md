...
Docker ve Docker Compose'un sisteminizde zaten kurulu olduğunu varsayacağız. Windows, Mac OS veya Linux'u ana sistem olarak kullanabilirsiniz. Desteklenen mimariler yalnızca x86-64 (masaüstü sistemler) ile sınırlı değildir, aynı zamanda düşük maliyetli ama yeterince güçlü bir web sunucusu olarak hizmet verebilen Raspberry Pi gibi ARM sistemlerini de içerir.

!!! note
    Sunucuda Gramps'ı kurmanıza gerek yoktur, çünkü Docker imajında bulunmaktadır.
...
- Let's Encrypt sertifikası ile Nginx Ters Proxy kullanmak
...
İlk çalıştırmada, uygulama size aşağıdakileri yapmanıza olanak tanıyan bir ilk çalıştırma sihirbazı gösterecektir:

- Sahip (admin) kullanıcı için bir hesap oluşturma
- Gerekli bazı yapılandırma seçeneklerini ayarlama
...
- [S3 üzerinde barındırılan medya dosyalarını](s3.md) kullanıyorsanız, S3 Medya Yükleyici Eklentisi'ni kullanabilirsiniz.
