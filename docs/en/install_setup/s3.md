...
When using [Docker Compose](deployment.md), the easiest option is to add all environment variables to the `environment:` block of the `grampsweb` service:
...
Note that only the Gramps Web configuration option carries the `GRAMPSWEB_` prefix; the `AWS_*` variables are read directly by the underlying `boto3` library and must stay unprefixed.
