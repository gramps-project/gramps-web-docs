...
## Graphiques d'arbre généalogique interactifs
...
Affichez tous les lieux de votre arbre sur une carte interactive et consultable. Recherchez de nouveaux lieux directement sur OpenStreetMap lors de la création d'un lieu, tracez les personnes de votre base de données géographiquement, et retracez la vie d'une seule personne en reliant ses événements par des lignes sur la carte.
...
Collaborez avec vos proches pour identifier des ancêtres sur de vieilles photos de famille. Grâce à la détection automatique des visages, le marquage des personnes se fait en seulement deux clics.
...
Chaque modification de votre arbre généalogique est enregistrée. Parcourez l'historique complet regroupé par transaction, plongez dans n'importe quel changement individuel pour voir exactement quels champs ont été ajoutés, supprimés ou modifiés, et annulez une transaction si cela s'avère être une erreur. La page de chaque objet dispose également de son propre onglet de révision montrant qui l'a modifié et quand.
...
Accédez à Gramps Web depuis n'importe quel appareil connecté à Internet. Vous pouvez télécharger des photos, créer ou modifier des enregistrements, montrer votre arbre généalogique à d'autres, ou rechercher ces noms de membres de la famille que vous ne pouvez pas vous rappeler lors de votre prochaine réunion de famille !
...
Changez la langue de l'interface entre plus de 50 langues traduites par la communauté Gramps.
...
- **Notifications et tâches en arrière-plan** – importations, exportations, rapports et reconstructions d'index s'exécutent en arrière-plan, avec progrès et erreurs collectés en un seul endroit
...
- **Vérification des données** – vérifiez votre arbre pour des dates implausibles et d'autres problèmes de données
- **Calendriers** – saisissez des dates dans le calendrier grégorien, julien, hébreu, républicain français, persan, islamique ou suédois
