...
!!! warning "Backend PostgreSQL obsolète"
    Le support du backend PostgreSQL (un arbre généalogique par base de données) sera supprimé dans une future version de l'API Gramps Web, car il n'est pas compatible avec l'hébergement de plusieurs arbres. Les backends SharedPostgreSQL et SQLite restent entièrement pris en charge. Pour les nouvelles installations, utilisez SharedPostgreSQL.

## Configuration du serveur PostgreSQL

Si vous souhaitez configurer une nouvelle base de données pour une utilisation avec le module complémentaire PostgreSQL, vous pouvez suivre les [instructions dans le Wiki Gramps](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL) pour configurer le serveur.
...
Encore une fois, si vous avez configuré le serveur PostgreSQL vous-même, vous pouvez suivre les [instructions dans le Wiki Gramps](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL) pour importer un arbre généalogique dans la base de données.
