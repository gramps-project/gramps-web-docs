...
`USER_DB_URI` | L'URL de la base de données de l'utilisateur. Toute URL compatible avec SQLAlchemy est autorisée.
...
`EMAIL_USE_SSL` | Booléen, indique s'il faut utiliser SSL/TLS implicite pour SMTP (v3.6.0+). Par défaut, c'est `True` si `EMAIL_USE_TLS` n'est pas explicitement défini. Utilisé typiquement avec le port 465.
`EMAIL_USE_STARTTLS` | Booléen, indique s'il faut utiliser STARTTLS explicite pour SMTP (v3.6.0+). Par défaut, c'est `False`. Utilisé typiquement avec le port 587 ou 25.
...
`OIDC_OPENID_CONFIG_URL` | Optionnel : URL vers le point de configuration OpenID Connect (si vous n'utilisez pas le standard `/.well-known/openid-configuration`)
