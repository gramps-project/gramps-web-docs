...
!!! info
    L'Assistant IA nécessite la version 2.5.0 ou supérieure de l'API Gramps Web et la version 24.10.0 ou supérieure de Gramps Web. La version 3.6.0 de l'API Gramps Web a introduit des capacités d'appel d'outils pour des interactions plus intelligentes.

La vue **Assistant** dans Gramps Web (si disponible dans votre installation, étiquetée "Chat" dans les versions plus anciennes) donne accès à un assistant IA qui peut répondre à des questions sur votre arbre généalogique.
...
2. L'assistant IA peut utiliser plusieurs approches pour trouver des réponses :
   - **Recherche sémantique** : Gramps Web identifie les objets dans votre arbre généalogique qui sont le plus susceptibles de contenir des informations pertinentes. Par exemple, si vous demandez "Quels sont les noms des enfants de John Doe ?", les familles avec John Doe comme père seront parmi les meilleurs résultats.
   - **Appel d'outils (API Gramps Web v3.6.0+)** : L'assistant peut interroger directement votre base de données en utilisant des outils spécialisés pour rechercher, filtrer des personnes/événements/familles/lieux selon des critères spécifiques, calculer les relations entre les individus et récupérer des informations détaillées.
...
Pendant que l'assistant travaille, des indicateurs montrent quels outils il utilise actuellement (par exemple, recherche de personnes, recherche de relations) afin que vous puissiez suivre son avancement dans la formulation de sa réponse. Les questions plus longues sont traitées comme des tâches en arrière-plan – vous pouvez naviguer ailleurs et revenir, et les progrès sont également reflétés dans [Notifications](notifications.md). Les réponses sont formatées avec Markdown (listes, emphase, liens, etc.) pour une lecture plus facile.

## Ce que vous pouvez demander

Avec les capacités d'appel d'outils introduites dans la version 3.6.0 de l'API Gramps Web, l'assistant IA peut désormais traiter des questions plus complexes :
...
- **Informations sur les lieux** : "Quels lieux se trouvent en France ?" ou "Parlez-moi de l'église St. Mary"
