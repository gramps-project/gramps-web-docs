...
Si aucune personne d'accueil n'a été définie, la page propose un bouton **Définir la Personne D'accueil** pour rechercher et en sélectionner une directement. Si l'arbre ne contient encore aucune personne, les utilisateurs ayant le rôle d'Éditeur ou supérieur voient à la place un bouton **Nouvelle Personne** – la première personne créée dans un arbre vide devient automatiquement votre personne d'accueil, de sorte que le graphique est affiché immédiatement.

## Types de graphiques

### Arbre des Ancêtres

Un graphique de parenté montrant les ancêtres de la personne sélectionnée. Les parents apparaissent à gauche (ou à droite, selon la mise en page), les grands-parents plus loin, et ainsi de suite.
...
Montre le chemin de relation entre deux personnes. La personne sélectionnée est un point d'extrémité ; cliquez sur n'importe quelle autre personne dans le graphique pour définir le deuxième point d'extrémité et afficher le chemin de relation le plus court entre elles.

!!! note
    La même personne peut apparaître plusieurs fois dans le graphique – par exemple, lorsqu'une personne a été mariée plusieurs fois et apparaît dans plusieurs unités familiales le long du chemin. C'est intentionnel, pas un bug.

### Graphique en éventail

Un graphique de parenté circulaire. Les ancêtres rayonnent vers l'extérieur à partir de la personne sélectionnée au centre.
...
La boîte de dialogue **Préférences** (icône d'engrenage dans la barre d'outils) vous permet d'ajuster les options suivantes, en fonction du type de graphique :
...
- **Max Degré de Séparation** – pour le Graphique de Relation, la longueur maximale du chemin à rechercher
- **Max Nombre d'Images affichées** – limite le nombre de photos de profil affichées dans le graphique pour des raisons de performance
...
Les paramètres de préférences de graphique sont stockés dans le stockage local du navigateur, de sorte qu'ils persistent entre les sessions sur le même appareil.
