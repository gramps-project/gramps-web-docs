...
Gramps Webを自分のサーバー（または仮想サーバー）でホストする最も便利な方法は、Docker Composeを使用することです。

DockerとDocker Composeがすでにシステムにインストールされていると仮定します。ホストシステムとしては、Windows、Mac OS、またはLinuxを使用できます。サポートされているアーキテクチャには、x86-64（デスクトップシステム）だけでなく、Raspberry PiのようなARMシステムも含まれており、低コストでありながら（十分に）強力なウェブサーバーとして機能します。
...
これにより、コンテナを再起動する際にすべての関連データが持続するように、6つの名前付きボリュームが生成されます。

!!! warning
    上記の設定により、ホストマシンのポート80でAPIが**SSL/TLS保護なし**で利用可能になります。これはローカルテストには使用できますが、インターネットに直接公開しないでください。これは完全に安全ではありません！

## ステップ2: SSL/TLSでの安全なアクセス

ウェブAPIは**必ず**HTTPS経由で公共のインターネットに提供される必要があります。いくつかのオプションがあります。例えば、
...
Gramps Webをローカルネットワーク内でのみ使用する予定の場合は、このステップをスキップできます。
...
初回実行時、アプリは初回ウィザードを表示し、以下のことを行うことができます。
...
- [S3にホストされたメディアファイル](s3.md)を使用する場合、S3メディアアップローダーアドオンを使用できます。
