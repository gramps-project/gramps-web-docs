...
デフォルトのDockerイメージを使用することで、すべての必要な構成はブラウザから行うことができます。しかし、デプロイメントによっては、サーバー構成をカスタマイズする必要がある場合があります。
...
- 各設定名を`GRAMPSWEB_`でプレフィックスし、環境変数の名前を取得します。
...
!!! warning "プレフィックスのない環境変数は非推奨です"
    歴史的な理由から、`TREE`、`SECRET_KEY`、`USER_DB_URI`、`POSTGRES_USER`、`POSTGRES_PASSWORD`、`MEDIA_BASE_DIR`、`SEARCH_INDEX_DIR`、`EMAIL_HOST`、`EMAIL_PORT`、`EMAIL_HOST_USER`、`EMAIL_HOST_PASSWORD`、`DEFAULT_FROM_EMAIL`、`BASE_URL`、および`STATIC_PATH`の一部の設定は、`GRAMPSWEB_`プレフィックスなしで環境変数を介して設定することができます。これは非推奨であり、起動時に警告がログに記録され、将来のリリースで動作しなくなります。常にプレフィックス付きの形式を使用してください。例：`GRAMPSWEB_TREE`ではなく`TREE`を使用します。
...
`TREE_ID` | シングルツリーモードで使用する家系図データベースのディレクトリ名（`TREE`が`*`に設定されていない場合）。設定すると、サーバーは表示名ではなくディレクトリ名でツリーを識別します。これはリネームに対してより堅牢です。APIを介してツリーの名前を変更したい場合は必須です。ディレクトリ名は`GET /api/trees/-`（`id`フィールド）を介して見つけることができます。
`SEARCH_INDEX_DB_URI` | 検索インデックスのデータベースURL。バックエンドとしては`sqlite`または`postgresql`のみが許可されます。デフォルトは`sqlite:///indexdir/search_index.db`で、スクリプトが実行されるパスに対して相対的に`indexdir`フォルダー内にSQLiteファイルを作成します。
`SEARCH_INDEX_DIR` | **非推奨**（代わりに`SEARCH_INDEX_DB_URI`を使用）。検索インデックスを含むディレクトリ。`SEARCH_INDEX_DB_URI`が未設定のときに設定すると、検索インデックスURLは`sqlite:///<SEARCH_INDEX_DIR>/search_index.db`として導出されます。
...
`EMAIL_USE_TLS` | **非推奨**（代わりに`EMAIL_USE_SSL`または`EMAIL_USE_STARTTLS`を使用）。ブール値、メール送信にTLSを使用するかどうか。デフォルトは`True`。STARTTLSを使用する場合は、これを`False`に設定し、ポート25とは異なるポートを使用します。
...
キー | 説明
----|-------------
`MEDIA_PREFIX_TREE` | 各ツリーのメディアファイル用に別のサブフォルダーを使用するかどうかのブール値。デフォルトは`False`ですが、マルチツリーセットアップでは`True`の使用を強く推奨します。
...
これらの設定は、外部プロバイダーとのOpenID Connect (OIDC)認証を使用したい場合に必要です。詳細なセットアップ手順と例については[OIDC Authentication](oidc.md)を参照してください。
...
これらの設定は、チャットやセマンティック検索などのAI駆動機能を使用したい場合に必要です。
...
`LLM_SYSTEM_PROMPT` | LLMチャットアシスタント用のカスタムシステムプロンプト（v3.6.0+）。未設定の場合、デフォルトの系譜最適化プロンプトを使用します。
