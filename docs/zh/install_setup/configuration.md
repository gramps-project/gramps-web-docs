...
!!! warning "未加前缀的环境变量已弃用"
    出于历史原因，一些设置 – `TREE`、`SECRET_KEY`、`USER_DB_URI`、`POSTGRES_USER`、`POSTGRES_PASSWORD`、`MEDIA_BASE_DIR`、`SEARCH_INDEX_DIR`、`EMAIL_HOST`、`EMAIL_PORT`、`EMAIL_HOST_USER`、`EMAIL_HOST_PASSWORD`、`DEFAULT_FROM_EMAIL`、`BASE_URL` 和 `STATIC_PATH` – 仍然可以通过不带 `GRAMPSWEB_` 前缀的环境变量进行设置。这是弃用的，在启动时记录警告，并将在未来的版本中停止工作。始终使用带前缀的形式，例如 `GRAMPSWEB_TREE` 而不是 `TREE`。

    请注意，这仅涉及环境变量。在配置文件中，设置名称始终使用未加前缀的形式。

!!! tip "检查弃用的选项"
    您的服务器仍依赖的弃用配置选项 – 例如未加前缀的环境变量、`SEARCH_INDEX_DIR` 或 `EMAIL_USE_TLS` – 在启动时会记录为警告。自 Gramps Web API 3.22 起，它们还会与其替代项及支持将被移除的版本一起列在 **系统信息** 页面顶部（在您以管理员身份登录时可以通过应用程序顶部的用户图标访问）。
...
`TREE_ID` | 在单树模式下使用的家谱数据库的目录名称（当 `TREE` 未设置为 `*` 时）。设置后，服务器通过其目录名称而不是显示名称来识别树，这对重命名更为稳健。如果您想通过 API 重命名树，则需要此项。目录名称可以通过 `GET /api/trees/-` 找到（`id` 字段）。
`SEARCH_INDEX_DB_URI` | 搜索索引的数据库 URL。仅允许 `sqlite` 或 `postgresql` 作为后端。默认为 `sqlite:///indexdir/search_index.db`，在运行脚本的路径下创建一个位于 `indexdir` 文件夹中的 SQLite 文件
`SEARCH_INDEX_DIR` | **已弃用**（请使用 `SEARCH_INDEX_DB_URI`）。包含搜索索引的目录。如果在未设置 `SEARCH_INDEX_DB_URI` 时设置，则搜索索引 URL 将推导为 `sqlite:///<SEARCH_INDEX_DIR>/search_index.db`。
...
`EMAIL_USE_TLS` | **已弃用**（请使用 `EMAIL_USE_SSL` 或 `EMAIL_USE_STARTTLS`）。布尔值，是否在发送电子邮件时使用 TLS。默认为 `True`。使用 STARTTLS 时，将此设置为 `False` 并使用与 25 不同的端口。
...
`MAX_THUMBNAIL_FILE_BYTES` | 设置缩略图的硬最大文件大小。默认为 `50 * 1024 * 1024`（50 MB）。提高此值可能会大幅增加内存使用，并可能导致内存不足崩溃或在内存中解压缩大文件时数据丢失。
...
如果您的家谱树托管在使用 SharedPostgreSQL 附加组件的 [PostgreSQL 数据库](postgres.md) 中，则需要这些设置。
...
`NEW_DB_BACKEND` | 新创建家谱树使用的数据库后端。必须是 `sqlite` 或 `sharedpostgresql` 之一。默认为 `sqlite`。值 `postgresql` 仍被接受但已弃用，因为 PostgreSQL 后端将在未来版本中移除。
`POSTGRES_HOST` | 在使用 SharedPostgreSQL 后端的多树设置中用于创建新树的 PostgreSQL 服务器的主机名
`POSTGRES_PORT` | 在使用 SharedPostgreSQL 后端的多树设置中用于创建新树的 PostgreSQL 服务器的端口

### OIDC 认证的设置

如果您想使用外部提供者进行 OpenID Connect (OIDC) 认证，则需要这些设置。有关详细的设置说明和示例，请参见 [OIDC 认证](oidc.md)。

键 | 描述
----|-------------
`OIDC_ENABLED` | 布尔值，是否启用 OIDC 认证。默认为 `False`。
`OIDC_ISSUER` | OIDC 提供者的发行者 URL（对于自定义 OIDC 提供者）
`OIDC_CLIENT_ID` | OAuth 客户端 ID（对于自定义 OIDC 提供者）
`OIDC_CLIENT_SECRET` | OAuth 客户端密钥（对于自定义 OIDC 提供者）
...
`LLM_MODEL` | 用于 OpenAI 兼容聊天 API 的模型。如果未设置（默认为），则禁用聊天。从 v3.6.0 开始，AI 助手使用具有工具调用能力的 Pydantic AI。
`VECTOR_EMBEDDING_MODEL` | 用于语义搜索向量嵌入的模型。当使用本地模型时，这必须是 [Sentence Transformers](https://sbert.net/) 模型名称。当使用远程 API（请参见 `VECTOR_EMBEDDING_BASE_URL`）时，这是传递给远程提供者的模型名称。如果未设置（默认为），则禁用语义搜索和聊天。
`VECTOR_EMBEDDING_BASE_URL` | 远程 OpenAI 兼容嵌入 API 的基本 URL（例如 Ollama、OpenAI、LiteLLM）。如果未设置（默认为），则使用本地 Sentence Transformers 模型。有关详细信息，请参见 [使用远程嵌入 API](chat.md#using-a-remote-embedding-api)。
