...
`TREE_ID` | 在单树模式下要使用的家谱数据库的目录名称（当 `TREE` 未设置为 `MULTI` 时）。设置后，服务器通过其目录名称而不是显示名称来识别树，这对重命名更为稳健。如果您想通过 API 重命名树，则需要此目录名称。可以通过 `GET /api/trees/-` 找到目录名称（`id` 字段）。
...
`BASE_URL` | API 可以访问的基本 URL（例如 `https://mygramps.mydomain.com/`）。这在构建正确的密码重置链接时是必要的
...
`CELERY_CONFIG` | 用于 Celery 后台任务队列的设置。有关可能的设置，请参见 [Celery](https://docs.celeryq.dev/en/stable/userguide/configuration.html)。
...
`MAX_THUMBNAIL_FILE_BYTES` | 设置缩略图的硬最大文件大小。默认为 `50 * 1024 * 1024`（50 MB）。提高此值可能会大幅增加内存使用，并可能导致内存溢出崩溃或在内存中解压缩大文件时数据丢失。
...
如果您想使用外部提供商的 OpenID Connect (OIDC) 认证，则需要这些设置。有关详细的设置说明和示例，请参见 [OIDC 认证](oidc.md)。

键 | 描述
----|-------------
`OIDC_ENABLED` | 布尔值，是否启用 OIDC 认证。默认为 `False`。
`OIDC_ISSUER` | OIDC 提供商发行者 URL（用于自定义 OIDC 提供商）
`OIDC_CLIENT_ID` | OAuth 客户端 ID（用于自定义 OIDC 提供商）
`OIDC_CLIENT_SECRET` | OAuth 客户端密钥（用于自定义 OIDC 提供商）
`OIDC_NAME` | 提供商的自定义显示名称。默认为 "OIDC"
...
`OIDC_AUTO_REDIRECT` | 布尔值，是否在仅配置一个提供商时自动重定向到 OIDC。默认为 `False`

#### 内置 OIDC 提供商

对于内置提供商（Google、Microsoft），使用以下设置：
...
这些设置允许您将来自身份提供商的 OIDC 组/角色映射到 Gramps Web 用户角色：

键 | 描述
----|-------------
`OIDC_ROLE_CLAIM` | OIDC 令牌中包含用户组/角色的声明名称。默认为 "groups"
`OIDC_GROUP_ADMIN` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Admin" 角色
`OIDC_GROUP_OWNER` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Owner" 角色
`OIDC_GROUP_EDITOR` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Editor" 角色
`OIDC_GROUP_CONTRIBUTOR` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Contributor" 角色
`OIDC_GROUP_MEMBER` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Member" 角色
`OIDC_GROUP_GUEST` | 来自您的 OIDC 提供商的组/角色名称，映射到 Gramps 的 "Guest" 角色
