...
请注意，通过环境设置的配置选项优先于配置文件中的选项。如果两者都存在，则环境变量“胜出”。

!!! warning "未加前缀的环境变量已被弃用"
    出于历史原因，一小部分设置 – `TREE`、`SECRET_KEY`、`USER_DB_URI`、`POSTGRES_USER`、`POSTGRES_PASSWORD`、`MEDIA_BASE_DIR`、`SEARCH_INDEX_DIR`、`EMAIL_HOST`、`EMAIL_PORT`、`EMAIL_HOST_USER`、`EMAIL_HOST_PASSWORD`、`DEFAULT_FROM_EMAIL`、`BASE_URL` 和 `STATIC_PATH` – 仍然可以通过不带 `GRAMPSWEB_` 前缀的环境变量进行设置。这已被弃用，在启动时会记录警告，并将在未来的版本中停止工作。始终使用带前缀的形式，例如 `GRAMPSWEB_TREE` 而不是 `TREE`。
...
`CORS_ORIGINS` | 允许来自的 CORS 请求的来源。默认情况下，所有请求都被拒绝。使用 `"*"` 以允许来自任何域的请求。
...
`EMAIL_USE_SSL` | 布尔值，是否对 SMTP 使用隐式 SSL/TLS（v3.6.0+）。如果未明确设置 `EMAIL_USE_TLS`，默认为 `True`。通常与端口 465 一起使用。
...
### 仅针对 PostgreSQL 后端数据库的设置

如果您已配置 Gramps 数据库与 [PostgreSQL 附加组件](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL) 一起使用，则需要此设置。
