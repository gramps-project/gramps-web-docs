...
!!! warning "未加前缀的环境变量已被弃用"
    出于历史原因，少数设置 – `TREE`、`SECRET_KEY`、`USER_DB_URI`、`POSTGRES_USER`、`POSTGRES_PASSWORD`、`MEDIA_BASE_DIR`、`SEARCH_INDEX_DIR`、`EMAIL_HOST`、`EMAIL_PORT`、`EMAIL_HOST_USER`、`EMAIL_HOST_PASSWORD`、`DEFAULT_FROM_EMAIL`、`BASE_URL` 和 `STATIC_PATH` – 仍然可以通过不带 `GRAMPSWEB_` 前缀的环境变量进行设置。这已被弃用，在启动时记录警告，并将在未来的版本中停止工作。始终使用带前缀的形式，例如 `GRAMPSWEB_TREE` 而不是 `TREE`。

    请注意，这仅涉及环境变量。在配置文件中，设置名称始终使用未加前缀的形式。

!!! tip "检查已弃用的选项"
    服务器仍依赖的已弃用配置选项 – 例如未加前缀的环境变量、`SEARCH_INDEX_DIR` 或 `EMAIL_USE_TLS` – 在启动时会记录为警告。自 Gramps Web API 3.22 起，它们还会在您以管理员身份登录时，在 **系统信息** 页面顶部列出，连同其替代项和将移除支持的版本（可通过顶部应用栏中的用户图标访问）。
...
`TREE_ID` | 在单树模式下使用的家谱数据库的目录名称（当 `TREE` 未设置为 `*` 时）。设置后，服务器通过其目录名称而不是显示名称来识别树，这对于重命名更为稳健。如果您想通过 API 重命名树，则需要此目录名称。可以通过 `GET /api/trees/-` 找到目录名称（`id` 字段）。
...
`EMAIL_USE_SSL` | 布尔值，是否为 SMTP 使用隐式 SSL/TLS（v3.6.0+）。如果未显式设置 `EMAIL_USE_TLS`，则默认为 `True`。通常与端口 465 一起使用。
`EMAIL_USE_STARTTLS` | 布尔值，是否为 SMTP 使用显式 STARTTLS（v3.6.0+）。默认为 `False`。通常与端口 587 或 25 一起使用。
...
`PILLOW_MAX_IMAGE_PIXELS` | 设置 PIL.Image.MAX_IMAGE_PIXELS 参数，指示处理后的图像可以包含的像素数量。有关详细信息，请参见 [文档](https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.MAX_IMAGE_PIXELS)。
`MAX_THUMBNAIL_FILE_BYTES` | 设置缩略图的硬最大文件大小。默认为 `50 * 1024 * 1024`（50 MB）。提高此限制可能会大幅增加内存使用，并可能导致内存不足崩溃或在内存中解压大文件时数据丢失。
...
`VECTOR_EMBEDDING_MODEL` | 用于语义搜索向量嵌入的模型。当使用本地模型时，这必须是 [Sentence Transformers](https://sbert.net/) 模型名称。当使用远程 API（请参见 `VECTOR_EMBEDDING_BASE_URL`）时，这是传递给远程提供者的模型名称。如果未设置（默认），则禁用语义搜索和聊天。
...
`VECTOR_EMBEDDING_API_KEY` | 用于经过身份验证的远程嵌入提供者的 API 密钥。仅在设置了 `VECTOR_EMBEDDING_BASE_URL` 并且提供者需要身份验证时需要。
