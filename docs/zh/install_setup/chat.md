...
这三个步骤将在下面依次描述。最后，所有者或管理员必须 [配置哪些用户可以访问聊天功能](users.md#configuring-who-can-use-ai-chat) 在管理用户设置中。

## 安装所需的依赖项

AI 聊天需要安装 Sentence Transformers 和 PyTorch 库。

Gramps Web 的标准 Docker 镜像已经为 `amd64`（例如 64 位桌面 PC）和 `arm64`（例如 64 位 Raspberry Pi）架构预先安装了它们。不幸的是，由于缺乏对 `armv7`（例如 32 位 Raspberry Pi）架构的 PyTorch 支持，AI 聊天不支持该架构。
...
- 当您更改模型时，必须手动重新创建您的树（或多树设置中的所有树）的语义搜索索引，否则您将遇到错误或无意义的结果。Gramps Web 会检测到配置的嵌入模型不再与现有索引匹配，并向管理员显示持久通知，提示他们从 [管理设置](../administration/settings.md#semantic-search-index) 触发完整的重新索引。
- 模型在准确性/通用性与计算时间/存储空间之间存在权衡。如果您在没有强大 GPU 的系统上运行 Gramps Web API，通常较大的模型在实践中会太慢。
- 除非您的整个数据库都是英文，并且所有用户仅期望用英文提问聊天，否则您将需要一个多语言嵌入模型，这种模型比纯英文模型更为稀缺。

如果模型不在本地缓存中，则在首次使用新配置启动 Gramps Web API 时将下载该模型。使用标准 Docker 镜像时，模型 `sentence-transformers/distiluse-base-multilingual-cased-v2` 已经在本地可用。该模型是一个不错的起点，并支持多语言输入。

请与社区分享关于不同模型的学习经验！

!!! info
    Sentence Transformers 库消耗大量内存，这可能导致工作进程被终止。作为经验法则，启用语义搜索时，每个 Gunicorn 工作进程大约消耗 200 MB 内存，每个 celery 工作进程即使在空闲时也消耗约 500 MB 内存，在计算嵌入时可高达 1 GB。有关限制内存使用的设置，请参见 [限制 CPU 和内存使用](cpu-limited.md)。此外，建议配置足够大的交换分区，以防止由于瞬时内存使用峰值导致的 OOM 错误。

## 使用远程嵌入 API

作为运行本地 Sentence Transformers 模型的替代方案，您可以使用远程 OpenAI 兼容的嵌入 API 进行语义搜索。如果您希望将嵌入计算卸载到单独的服务（例如 [Ollama](https://ollama.com/)）、使用云嵌入提供者（例如 OpenAI），或避免将 Sentence Transformers 和 PyTorch 库加载到内存中，这将非常有用。

远程 API 必须与 [OpenAI 嵌入端点](https://platform.openai.com/docs/api-reference/embeddings) (`/v1/embeddings`) 兼容。

要使用远程嵌入 API，请设置以下配置选项（详见 [服务器配置](configuration.md)）：
...
!!! warning
    更改嵌入模型需要重新索引您树中的所有记录（或多树设置中的所有树），因为不同的模型生成不同维度的向量。

## 设置 LLM 提供者

与 LLM 的通信使用 Pydantic AI 框架，该框架支持 OpenAI 兼容的 API。这允许通过 Ollama 使用本地部署的 LLM（参见 [Ollama OpenAI 兼容性](https://ollama.com/blog/openai-compatibility)）或托管 API，如 OpenAI、Anthropic 或 Hugging Face TGI（文本生成推理）。LLM 通过配置参数 `LLM_MODEL` 和 `LLM_BASE_URL` 进行配置。

### 通过 OpenAI API 使用托管的 LLM
...
[Ollama](https://ollama.com/) 是本地运行 LLM 的便捷方式。请查阅 Ollama 文档以获取详细信息。请注意，LLM 需要大量计算资源，除了最小的模型外，其他模型在没有 GPU 支持的情况下可能会太慢。您可以尝试 [`tinyllama`](https://ollama.com/library/tinyllama) 是否满足您的需求。如果不满足，请尝试更大的模型。请与社区分享任何经验！
...
然后将 `LLM_BASE_URL` 配置参数设置为 `http://ollama:11434/v1`。将 `LLM_MODEL` 设置为 Ollama 支持的模型，并在您的容器中使用 `ollama pull <model>` 将其拉取。最后，将 `OPENAI_API_KEY` 设置为 `ollama`。

要排查 Ollama 的问题，您可以通过在 Ollama 服务环境中设置环境变量 `OLLAMA_DEBUG=1` 来启用调试日志。

!!! info
    如果您使用 Ollama 进行 Gramps Web AI 聊天，请通过补充任何缺失的细节来支持社区。
