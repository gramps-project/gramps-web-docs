...
Øverst på siden vises den aktuelle brug i forhold til eventuelle konfigurerede grænser:
...
Kvoterne er sat af serveradministratoren; se [Serverkonfiguration](../install_setup/configuration.md) for detaljer.

## Importér data

Importsektionen giver dig mulighed for at uploade en familie træfil eller et mediearkiv. Se [Importér data](import.md) for fulde instruktioner.
...
Muliggør upload af en ZIP-fil med mediefiler for at udfylde manglende filer. Se [Importér data](import.md) for fulde instruktioner.
...
!!! note
    Omdøbning af træet fungerer kun i en [multi-træ opsætning](../install_setup/multi-tree.md) eller når `TREE_ID` er eksplicit indstillet i [serverkonfigurationen](../install_setup/configuration.md). På en standard installation med et enkelt træ uden `TREE_ID` indstillet, vil dette medføre en fejl.
...
Dette værktøj tjekker Gramps-databasen for interne inkonsistenser og retter dem, det kan — svarende til værktøjet [Tjek og reparation af database](https://www.gramps-project.org/wiki/index.php/Gramps_5.2_Wiki_Manual_-_Tools#Check_and_Repair_Database) i Gramps Desktop.

Klik på **Tjek og reparation** og vent på, at fremdriftsindikatoren fuldføres. Resultatet vises under knappen:
...
Kør dette værktøj, hvis du oplever uventede fejl eller adfærd, der kan være forårsaget af databaseinkonsistenser, såsom manglende relationer mellem objekter.
