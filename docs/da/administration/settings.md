...
Importsektionen giver dig mulighed for at uploade en slægtstræfil eller et mediearkiv. Se [Importer data](import.md) for fulde instruktioner.
...
Indstil en brugerdefineret titel for applikationen. Hvis den er indstillet, overskriver denne titlen for slægtstræet i browserens titelbar og den øverste app-bar.

Indtast en titel og klik på **Gem**. Lad det stå tomt for at bruge standarden (slægtstrænavnet).
...
Dette værktøj tjekker Gramps-databasen for interne uoverensstemmelser og retter dem, det kan – svarende til værktøjet [Tjek og Reparer Database](https://www.gramps-project.org/wiki/index.php/Gramps_5.2_Wiki_Manual_-_Tools#Check_and_Repair_Database) i Gramps Desktop.

Klik på **Tjek og Reparer** og vent på, at fremdriftsindikatoren er færdig. Resultatet vises under knappen:
...
Kør dette værktøj, hvis du støder på uventede fejl eller adfærd, der kan være forårsaget af databaseinkonsistenser, såsom manglende relationer mellem objekter.
...
Se [Tags](../user-guide/tags.md) for hvordan tags bruges i hele Gramps Web, inklusive de særlige `Blog` og `ToDo` tags.
...
Du vil blive bedt om at re-autoriserer (logge ind igen) for at bekræfte handlingen. Sletningen kører som en baggrundsopgave, og en fremdriftsindikator vises.

!!! warning
    Sletning af kun et delmængde af objekttyper (i stedet for alle objekter på én gang) kan tage meget lang tid for store træer, da serveren skal tjekke og opdatere alle relationer mellem objekter.
...
Nulstiller træet til at matche en uploadet Gramps XML (`.gramps`) backupfil, tilføjer, opdaterer og sletter objekter efter behov, så træet ender med at være identisk med backupen.

!!! danger
    Dette er en destruktiv erstatning, ikke en sammensmeltning. Ethvert eksisterende objekt, der ikke er til stede i den uploadede backup, slettes.

Upload en `.gramps` fil, og klik derefter på **Forhåndsvis Gendannelse**. Du vil blive bedt om at re-autoriserer, hvis din session ikke er frisk nok. En forhåndsvisning kører som en baggrundsopgave, og når den er færdig, åbner den en dialog, der opsummerer ændringerne pr. objekttype (personer, familier, begivenheder, steder, citater, kilder, arkiver, medieobjekter, noter, tags):
