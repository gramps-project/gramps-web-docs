...
!!! warning "Uden præfiksede miljøvariabler er forældede"
    Af historiske grunde kan en håndfuld indstillinger – `TREE`, `SECRET_KEY`, `USER_DB_URI`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `MEDIA_BASE_DIR`, `SEARCH_INDEX_DIR`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `DEFAULT_FROM_EMAIL`, `BASE_URL`, og `STATIC_PATH` – stadig indstilles via en miljøvariabel *uden* `GRAMPSWEB_` præfiks. Dette er forældet, logger en advarsel ved opstart, og vil stoppe med at fungere i en fremtidig udgivelse. Brug altid den præfikserede form, f.eks. `GRAMPSWEB_TREE` i stedet for `TREE`.
...
`TREE_ID` | Mappenavnet på den familie trædatabase, der skal bruges i enkelt-træ tilstand (når `TREE` ikke er indstillet til `*`). Når det er indstillet, identificerer serveren træet efter dets mappenavn snarere end dets visningsnavn, hvilket er mere robust over for omdøbninger. Påkrævet, hvis du vil omdøbe træet via API'en. Mappenavnet kan findes via `GET /api/trees/-` (feltet `id`).
...
`SEARCH_INDEX_DIR` | **Forældet** (brug `SEARCH_INDEX_DB_URI` i stedet). Mappe, der indeholder søgeindekset. Hvis det er indstillet, mens `SEARCH_INDEX_DB_URI` ikke er indstillet, udledes søgeindeks-URL'en som `sqlite:///<SEARCH_INDEX_DIR>/search_index.db`.
...
`CELERY_CONFIG` | Indstillinger for Celery baggrundsopgavekø. Se [Celery](https://docs.celeryq.dev/en/stable/userguide/configuration.html) for mulige indstillinger.
...
`MAX_THUMBNAIL_FILE_BYTES` | Indstiller en hård maksimal filstørrelse for thumbnails. Standard til `50 * 1024 * 1024` (50 MB). At hæve den kan i høj grad øge hukommelsesforbruget og kan føre til hukommelsessvigt eller datatab, hvis store filer dekomprimeres i hukommelsen.


!!! info
    Når du bruger miljøvariabler til konfiguration, skal booleske indstillinger som `EMAIL_USE_SSL` være enten strengen `true` eller `false` (case sensitive!).


### Indstillinger kun for PostgreSQL backend-database

Dette er påkrævet, hvis du har konfigureret din Gramps-database til at arbejde med [PostgreSQL-tilføjelsen](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL).
...
`OIDC_ISSUER` | OIDC-udbyderens udsteder-URL (til tilpassede OIDC-udbydere)
`OIDC_CLIENT_ID` | OAuth-klient-ID (til tilpassede OIDC-udbydere)
`OIDC_CLIENT_SECRET` | OAuth-klienthemmelighed (til tilpassede OIDC-udbydere)
`OIDC_NAME` | Tilpasset visningsnavn for udbyderen. Standard til "OIDC"
...
`OIDC_OPENID_CONFIG_URL` | Valgfri: URL til OpenID Connect konfigurationsendpoint (hvis ikke standard `/.well-known/openid-configuration`)
...
`VECTOR_EMBEDDING_MODEL` | Modellen, der skal bruges til semantisk søgning vektorembedninger. Når du bruger en lokal model, skal dette være et [Sentence Transformers](https://sbert.net/) modellenavn. Når du bruger en fjern API (se `VECTOR_EMBEDDING_BASE_URL`), er dette modellenavnet, der sendes til den fjerne udbyder. Hvis ikke indstillet (standard), er semantisk søgning og chat deaktiveret.
...
`VECTOR_EMBEDDING_API_KEY` | API-nøgle til autentificerede fjernaftaler. Kun nødvendig, når `VECTOR_EMBEDDING_BASE_URL` er indstillet, og udbyderen kræver autentificering.
`LLM_MAX_CONTEXT_LENGTH` | Tegnbegrænsning for familie trækontexten, der gives til LLM. Standard til 50000.
`LLM_SYSTEM_PROMPT` | Tilpasset systemprompt til LLM chatassistent (v3.6.0+). Hvis ikke indstillet, bruges den standard genealogisk optimerede prompt.
