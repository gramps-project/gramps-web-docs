...
Đỉnh trang hiển thị mức sử dụng hiện tại so với bất kỳ giới hạn nào đã được cấu hình:

- **Người** – số lượng đối tượng cá nhân trong cây so với tối đa đã cấu hình (∞ nếu không giới hạn)
...
Cho phép tải lên một tệp ZIP của các tệp phương tiện để lấp đầy các tệp bị thiếu. Xem [Nhập dữ liệu](import.md) để biết hướng dẫn đầy đủ.
...
- **Tái tạo chỉ mục tìm kiếm ngữ nghĩa** – xây dựng lại toàn bộ chỉ mục ngữ nghĩa từ đầu. Điều này tốn nhiều tài nguyên tính toán và có thể mất nhiều thời gian.
...
Chạy công cụ này nếu bạn gặp phải lỗi hoặc hành vi không mong muốn có thể do các bất thường trong cơ sở dữ liệu, chẳng hạn như các mối quan hệ bị thiếu giữa các đối tượng.
