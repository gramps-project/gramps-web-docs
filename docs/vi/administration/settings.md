...
Trang **Cài đặt > Quản trị** có thể truy cập thông qua biểu tượng người dùng ở thanh công cụ trên cùng của ứng dụng. Nó chỉ có sẵn cho người dùng có vai trò Chủ sở hữu hoặc Quản trị viên và cung cấp các công cụ để quản lý cơ sở dữ liệu cây gia đình.
...
Hạn mức được thiết lập bởi quản trị viên máy chủ; xem [Cấu hình máy chủ](../install_setup/configuration.md) để biết thêm chi tiết.
...
Nhấp vào **Cập nhật chỉ mục tìm kiếm** để kích hoạt một lần xây dựng lại hoàn toàn. Một chỉ báo tiến trình sẽ được hiển thị trong khi tác vụ chạy ở chế độ nền. Điều này thường chỉ cần thiết sau khi nâng cấp máy chủ.

### Chỉ mục tìm kiếm ngữ nghĩa

Nếu máy chủ đã [bật tìm kiếm ngữ nghĩa (được hỗ trợ bởi AI)](../install_setup/configuration.md), một phần bổ sung sẽ xuất hiện với hai hành động:
...
!!! note
    Việc đổi tên cây chỉ hoạt động trong một [cài đặt nhiều cây](../install_setup/multi-tree.md) hoặc khi `TREE_ID` được thiết lập rõ ràng trong [cấu hình máy chủ](../install_setup/configuration.md). Trên một cài đặt cây đơn mặc định mà không có `TREE_ID` được thiết lập, điều này sẽ gây ra lỗi.

Điều này cho phép thay đổi tên của cơ sở dữ liệu cây gia đình Gramps. Nhập một tên mới và nhấp vào **Đổi tên** để áp dụng.

!!! tip
    Nếu bạn chỉ muốn thay đổi tên hiển thị trong thanh công cụ ứng dụng mà không đổi tên cơ sở dữ liệu, hãy sử dụng cài đặt [Tiêu đề ứng dụng](#app-title) thay thế.

### Thông tin Nhà Nghiên Cứu

Thiết lập tên, địa chỉ và thông tin liên hệ của nhà nghiên cứu chính. Thông tin này được nhúng trong các xuất khẩu (ví dụ: tệp GEDCOM).
...
Sử dụng các bộ chọn màu để chọn màu, sau đó nhấp vào **Lưu**. Nhấp vào **Đặt lại** để quay lại mặc định.

### Tiêu đề ứng dụng

Thiết lập một tiêu đề tùy chỉnh cho ứng dụng. Nếu được thiết lập, điều này sẽ ghi đè tên cây gia đình trong thanh tiêu đề trình duyệt và thanh công cụ trên cùng.
...
Nhấp vào **Kiểm tra và Sửa chữa** và chờ cho chỉ báo tiến trình hoàn thành. Kết quả sẽ được hiển thị bên dưới nút:
...
Chạy công cụ này nếu bạn gặp phải các lỗi hoặc hành vi bất ngờ có thể do bất thường trong cơ sở dữ liệu, chẳng hạn như các mối quan hệ bị thiếu giữa các đối tượng.
...
Nhấp vào **Thẻ Mới** để tạo một thẻ. Sử dụng các điều khiển bên cạnh một thẻ hiện có để đổi tên nó (biểu tượng bút chì), thay đổi màu sắc của nó (bộ chọn màu), hoặc xóa nó (biểu tượng xóa).

!!! note
    Việc xóa một thẻ sẽ xóa nó khỏi tất cả các đối tượng mà nó đã được áp dụng.
...
!!! warning
    Việc chỉ xóa một tập hợp con của các loại đối tượng (thay vì tất cả các đối tượng cùng một lúc) có thể mất rất nhiều thời gian cho các cây lớn, vì máy chủ phải kiểm tra và cập nhật tất cả các mối quan hệ giữa các đối tượng.
...
Nếu có bất kỳ đối tượng nào sẽ bị xóa, hộp thoại sẽ cảnh báo số lượng. Xem lại tóm tắt, sau đó nhấp vào **Khôi phục** để áp dụng các thay đổi, hoặc **Hủy** để hủy bỏ.

!!! note
    Chỉ dữ liệu đối tượng và các tham chiếu phương tiện được khôi phục. Các tệp phương tiện nhị phân và siêu dữ liệu cây (người mặc định, dấu trang, nhóm tên) không bị ảnh hưởng. Khôi phục các tệp phương tiện bị thiếu riêng biệt qua [Nhập kho lưu trữ phương tiện](#import-media-archive) nếu cần.

!!! tip
    Sử dụng điều này để quay lại một cây về một bản sao lưu Gramps XML đã biết tốt, ví dụ như sau một lần nhập xấu hoặc một chỉnh sửa hàng loạt không mong muốn.
