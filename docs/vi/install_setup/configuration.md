...
Lưu ý rằng các tùy chọn cấu hình được thiết lập thông qua môi trường có ưu tiên hơn so với những cái trong tệp cấu hình. Nếu cả hai đều có mặt, biến môi trường sẽ "thắng".

!!! warning "Các biến môi trường không có tiền tố đã lỗi thời"
    Vì lý do lịch sử, một số cài đặt – `TREE`, `SECRET_KEY`, `USER_DB_URI`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `MEDIA_BASE_DIR`, `SEARCH_INDEX_DIR`, `EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD`, `DEFAULT_FROM_EMAIL`, `BASE_URL`, và `STATIC_PATH` – vẫn có thể được thiết lập thông qua một biến môi trường *không có* tiền tố `GRAMPSWEB_`. Điều này đã lỗi thời, ghi lại một cảnh báo khi khởi động, và sẽ ngừng hoạt động trong một phiên bản tương lai. Luôn sử dụng dạng có tiền tố, ví dụ `GRAMPSWEB_TREE` thay vì `TREE`.
...
!!! tip "Kiểm tra các tùy chọn đã lỗi thời"
    Các tùy chọn cấu hình đã lỗi thời mà máy chủ của bạn vẫn phụ thuộc vào – chẳng hạn như các biến môi trường không có tiền tố, `SEARCH_INDEX_DIR`, hoặc `EMAIL_USE_TLS` – được ghi lại dưới dạng cảnh báo khi khởi động. Kể từ Gramps Web API 3.22, chúng cũng được liệt kê, cùng với sự thay thế của chúng và phiên bản mà hỗ trợ sẽ bị loại bỏ, ở đầu trang **Thông tin hệ thống** (có thể truy cập thông qua biểu tượng người dùng ở thanh ứng dụng trên cùng) khi bạn đăng nhập với tư cách là quản trị viên.
...
`TREE_ID` | Tên thư mục của cơ sở dữ liệu cây gia đình để sử dụng trong chế độ một cây (khi `TREE` không được đặt thành `*`). Khi được đặt, máy chủ xác định cây theo tên thư mục của nó thay vì tên hiển thị của nó, điều này mạnh mẽ hơn khi đổi tên. Bắt buộc nếu bạn muốn đổi tên cây qua API. Tên thư mục có thể được tìm thấy qua `GET /api/trees/-` (trường `id`).
...
`SEARCH_INDEX_DIR` | **Đã lỗi thời** (sử dụng `SEARCH_INDEX_DB_URI` thay thế). Thư mục chứa chỉ mục tìm kiếm. Nếu được đặt trong khi `SEARCH_INDEX_DB_URI` không được đặt, URL chỉ mục tìm kiếm được suy ra là `sqlite:///<SEARCH_INDEX_DIR>/search_index.db`.
`STATIC_PATH` | Đường dẫn để phục vụ các tệp tĩnh từ (ví dụ: một giao diện web tĩnh)
`BASE_URL` | URL gốc nơi API có thể được truy cập (ví dụ: `https://mygramps.mydomain.com/`). Điều này cần thiết ví dụ để xây dựng các liên kết đặt lại mật khẩu chính xác
`CORS_ORIGINS` | Các nguồn mà yêu cầu CORS được phép từ. Mặc định, tất cả đều bị từ chối. Sử dụng `"*"` để cho phép yêu cầu từ bất kỳ miền nào.
...
`EMAIL_USE_TLS` | **Đã lỗi thời** (sử dụng `EMAIL_USE_SSL` hoặc `EMAIL_USE_STARTTLS` thay thế). Boolean, có sử dụng TLS để gửi email hay không. Mặc định là `True`. Khi sử dụng STARTTLS, đặt điều này thành `False` và sử dụng một cổng khác với 25.
`EMAIL_USE_SSL` | Boolean, có sử dụng SSL/TLS ngầm cho SMTP (v3.6.0+). Mặc định là `True` nếu `EMAIL_USE_TLS` không được đặt rõ ràng. Thường được sử dụng với cổng 465.
...
`THUMBNAIL_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ đệm hình thu nhỏ. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể.
`REQUEST_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ đệm yêu cầu. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể.
`PERSISTENT_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ đệm bền vững, được sử dụng ví dụ cho telemetry. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể.
...
`MAX_THUMBNAIL_FILE_BYTES` | Thiết lập kích thước tệp tối đa cứng cho hình thu nhỏ. Mặc định là `50 * 1024 * 1024` (50 MB). Tăng nó có thể làm tăng đáng kể mức sử dụng bộ nhớ và có thể dẫn đến sự cố do hết bộ nhớ hoặc mất dữ liệu nếu các tệp lớn được giải nén trong bộ nhớ.
...
### Các cài đặt chỉ dành cho cơ sở dữ liệu backend PostgreSQL

Điều này là cần thiết nếu bạn đã cấu hình cơ sở dữ liệu Gramps của mình để hoạt động với [PostgreSQL addon](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL).
...
### Các cài đặt liên quan đến việc lưu trữ nhiều cây
...
Chìa khóa | Mô tả
----|-------------
`MEDIA_PREFIX_TREE` | Boolean, có sử dụng một thư mục con riêng cho các tệp phương tiện của mỗi cây hay không. Mặc định là `False`, nhưng rất khuyến khích sử dụng `True` trong cài đặt nhiều cây
...
### Các cài đặt chỉ dành cho các tính năng AI
...
`LLM_MODEL` | Mô hình để sử dụng cho API trò chuyện tương thích với OpenAI. Nếu không được đặt (mặc định), trò chuyện sẽ bị vô hiệu hóa. Tính từ v3.6.0, trợ lý AI sử dụng Pydantic AI với khả năng gọi công cụ.
`VECTOR_EMBEDDING_MODEL` | Mô hình để sử dụng cho nhúng vector tìm kiếm ngữ nghĩa. Khi sử dụng một mô hình cục bộ, điều này phải là tên mô hình [Sentence Transformers](https://sbert.net/). Khi sử dụng một API từ xa (xem `VECTOR_EMBEDDING_BASE_URL`), đây là tên mô hình được truyền đến nhà cung cấp từ xa. Nếu không được đặt (mặc định), tìm kiếm ngữ nghĩa và trò chuyện sẽ bị vô hiệu hóa.
`VECTOR_EMBEDDING_BASE_URL` | URL gốc cho một API nhúng tương thích với OpenAI từ xa (ví dụ: Ollama, OpenAI, LiteLLM). Nếu không được đặt (mặc định), một mô hình Sentence Transformers cục bộ được sử dụng. Xem [Sử dụng một API nhúng từ xa](chat.md#using-a-remote-embedding-api) để biết chi tiết.
`VECTOR_EMBEDDING_API_KEY` | Khóa API cho các nhà cung cấp nhúng từ xa được xác thực. Chỉ cần thiết khi `VECTOR_EMBEDDING_BASE_URL` được đặt và nhà cung cấp yêu cầu xác thực.
`LLM_MAX_CONTEXT_LENGTH` | Giới hạn ký tự cho ngữ cảnh cây gia đình được cung cấp cho LLM. Mặc định là 50000.
`LLM_SYSTEM_PROMPT` | Lời nhắc hệ thống tùy chỉnh cho trợ lý trò chuyện LLM (v3.6.0+). Nếu không được đặt, sử dụng lời nhắc tối ưu hóa geneaology mặc định.
