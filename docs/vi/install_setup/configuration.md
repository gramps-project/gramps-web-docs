...
- Sử dụng dấu gạch dưới đôi cho các cài đặt từ điển lồng nhau, ví dụ `GRAMPSWEB_THUMBNAIL_CACHE_CONFIG__CACHE_DEFAULT_TIMEOUT` sẽ thiết lập giá trị cho tùy chọn cấu hình `THUMBNAIL_CACHE_CONFIG['CACHE_DEFAULT_TIMEOUT']`

Lưu ý rằng các tùy chọn cấu hình được thiết lập qua môi trường có ưu tiên hơn các tùy chọn trong tệp cấu hình. Nếu cả hai đều có mặt, biến môi trường "chiến thắng".
...
!!! warning "Các biến môi trường không có tiền tố đã lỗi thời"
    ...
    Lưu ý rằng điều này chỉ liên quan đến các biến môi trường. Trong tệp cấu hình, tên cài đặt luôn được sử dụng mà không có tiền tố.

!!! tip "Kiểm tra các tùy chọn đã lỗi thời"
    Các tùy chọn cấu hình đã lỗi thời mà máy chủ của bạn vẫn phụ thuộc vào – chẳng hạn như các biến môi trường không có tiền tố, `SEARCH_INDEX_DIR`, hoặc `EMAIL_USE_TLS` – được ghi lại dưới dạng cảnh báo khi khởi động. Kể từ Gramps Web API 3.22, chúng cũng được liệt kê, cùng với sự thay thế của chúng và phiên bản mà hỗ trợ sẽ bị loại bỏ, ở đầu trang **Thông tin hệ thống** (có thể truy cập qua biểu tượng người dùng trong thanh ứng dụng trên cùng) khi bạn đăng nhập với tư cách là quản trị viên.
...
`SECRET_KEY` | Khóa bí mật cho flask. Khóa bí mật không được chia sẻ công khai. Thay đổi nó sẽ làm mất hiệu lực tất cả các mã thông báo truy cập.
...
Khóa | Mô tả
----|-------------
`MEDIA_BASE_DIR` | Đường dẫn sử dụng làm thư mục gốc cho các tệp phương tiện, ghi đè thư mục gốc phương tiện được thiết lập trong Gramps. Khi sử dụng [S3](s3.md), phải có dạng `s3://<bucket_name>`
`TREE_ID` | Tên thư mục của cơ sở dữ liệu cây gia đình để sử dụng trong chế độ một cây (khi `TREE` không được đặt thành `*`). Khi được thiết lập, máy chủ xác định cây theo tên thư mục của nó thay vì tên hiển thị của nó, điều này mạnh mẽ hơn khi đổi tên. Bắt buộc nếu bạn muốn đổi tên cây qua API. Tên thư mục có thể được tìm thấy qua `GET /api/trees/-` (trường `id`).
`SEARCH_INDEX_DB_URI` | URL cơ sở dữ liệu cho chỉ mục tìm kiếm. Chỉ `sqlite` hoặc `postgresql` được phép làm backend. Mặc định là `sqlite:///indexdir/search_index.db`, tạo một tệp SQLite trong thư mục `indexdir` tương đối với đường dẫn nơi tập lệnh được chạy
`SEARCH_INDEX_DIR` | **Đã lỗi thời** (sử dụng `SEARCH_INDEX_DB_URI` thay vào đó). Thư mục chứa chỉ mục tìm kiếm. Nếu được thiết lập trong khi `SEARCH_INDEX_DB_URI` không được thiết lập, URL chỉ mục tìm kiếm được suy ra là `sqlite:///<SEARCH_INDEX_DIR>/search_index.db`.
`STATIC_PATH` | Đường dẫn để phục vụ các tệp tĩnh (ví dụ: một giao diện web tĩnh)
`BASE_URL` | URL gốc nơi API có thể được truy cập (ví dụ: `https://mygramps.mydomain.com/`). Điều này là cần thiết ví dụ như để xây dựng các liên kết đặt lại mật khẩu chính xác
...
`EMAIL_USE_TLS` | **Đã lỗi thời** (sử dụng `EMAIL_USE_SSL` hoặc `EMAIL_USE_STARTTLS` thay vào đó). Boolean, có sử dụng TLS để gửi email hay không. Mặc định là `True`. Khi sử dụng STARTTLS, đặt điều này thành `False` và sử dụng một cổng khác với 25.
...
`PERSISTENT_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ cache bền, được sử dụng ví dụ như cho telemetry. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể có.
...
`PILLOW_MAX_IMAGE_PIXELS` | Thiết lập tham số PIL.Image.MAX_IMAGE_PIXELS, chỉ ra số pixel mà hình ảnh đã xử lý có thể chứa. Xem [docs](https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.MAX_IMAGE_PIXELS) để biết chi tiết.
`MAX_THUMBNAIL_FILE_BYTES` | Thiết lập kích thước tệp tối đa cứng cho các hình thu nhỏ. Mặc định là `50 * 1024 * 1024` (50 MB). Tăng nó có thể làm tăng đáng kể mức sử dụng bộ nhớ và có thể dẫn đến sự cố hết bộ nhớ hoặc mất dữ liệu nếu các tệp lớn được giải nén trong bộ nhớ.


!!! info
    Khi sử dụng biến môi trường cho cấu hình, các tùy chọn boolean như `EMAIL_USE_SSL` phải là chuỗi `true` hoặc `false` (chú ý chữ hoa chữ thường!).


### Cài đặt cho cơ sở dữ liệu PostgreSQL

Các cài đặt này là bắt buộc nếu các cây gia đình của bạn được lưu trữ trong một [cơ sở dữ liệu PostgreSQL](postgres.md) sử dụng addon SharedPostgreSQL.
...
`NEW_DB_BACKEND` | Backend cơ sở dữ liệu để sử dụng cho các cây gia đình mới được tạo. Phải là một trong `sqlite` hoặc `sharedpostgresql`. Mặc định là `sqlite`. Giá trị `postgresql` vẫn được chấp nhận nhưng đã lỗi thời, vì backend PostgreSQL sẽ bị loại bỏ trong một phiên bản tương lai.
...
`OIDC_ISSUER` | URL nhà cung cấp OIDC (cho các nhà cung cấp OIDC tùy chỉnh)
`OIDC_CLIENT_ID` | ID khách hàng OAuth (cho các nhà cung cấp OIDC tùy chỉnh)
`OIDC_CLIENT_SECRET` | Mật khẩu khách hàng OAuth (cho các nhà cung cấp OIDC tùy chỉnh)
...
`LLM_MODEL` | Mô hình để sử dụng cho API trò chuyện tương thích với OpenAI. Nếu không được thiết lập (mặc định), trò chuyện bị vô hiệu hóa. Từ phiên bản v3.6.0, trợ lý AI sử dụng Pydantic AI với khả năng gọi công cụ.
`VECTOR_EMBEDDING_MODEL` | Mô hình để sử dụng cho nhúng vector tìm kiếm ngữ nghĩa. Khi sử dụng một mô hình cục bộ, điều này phải là tên mô hình [Sentence Transformers](https://sbert.net/). Khi sử dụng một API từ xa (xem `VECTOR_EMBEDDING_BASE_URL`), đây là tên mô hình được truyền cho nhà cung cấp từ xa. Nếu không được thiết lập (mặc định), tìm kiếm ngữ nghĩa và trò chuyện bị vô hiệu hóa.
`VECTOR_EMBEDDING_BASE_URL` | URL gốc cho một API nhúng tương thích với OpenAI từ xa (ví dụ: Ollama, OpenAI, LiteLLM). Nếu không được thiết lập (mặc định), một mô hình Sentence Transformers cục bộ được sử dụng. Xem [Sử dụng API nhúng từ xa](chat.md#using-a-remote-embedding-api) để biết chi tiết.
`VECTOR_EMBEDDING_API_KEY` | Khóa API cho các nhà cung cấp nhúng từ xa được xác thực. Chỉ cần thiết khi `VECTOR_EMBEDDING_BASE_URL` được thiết lập và nhà cung cấp yêu cầu xác thực.
`LLM_MAX_CONTEXT_LENGTH` | Giới hạn ký tự cho ngữ cảnh cây gia đình được cung cấp cho LLM. Mặc định là 50000.
`LLM_SYSTEM_PROMPT` | Lời nhắc hệ thống tùy chỉnh cho trợ lý trò chuyện LLM (v3.6.0+). Nếu không được thiết lập, sử dụng lời nhắc tối ưu hóa cho gia phả mặc định.
