...
Việc sử dụng PostgreSQL trong Docker với Gramps chỉ phức tạp bởi thực tế là các hình ảnh PostgreSQL mặc định không có bất kỳ ngôn ngữ địa phương nào được cài đặt, điều này tuy nhiên là cần thiết cho Gramps để sắp xếp các đối tượng theo ngôn ngữ địa phương. Tùy chọn dễ nhất là sử dụng hình ảnh `gramps-postgres` được phát hành trong [kho này](https://github.com/DavidMStraub/gramps-postgres-docker/). Để sử dụng nó, thêm phần sau vào tệp `docker-compose.yml` của bạn:
```yaml
  postgres_gramps:
...
```
và cũng thêm `postgres_data:` như một khóa dưới phần `volumes:` của tệp YAML này. Hình ảnh này chứa một cơ sở dữ liệu riêng cho dữ liệu gia phả của Gramps và cho cơ sở dữ liệu người dùng Gramps; mỗi cái có thể có mật khẩu riêng.
...
```yaml
      # addon PostgreSQL giả định tên cây là
      # bằng với tên cơ sở dữ liệu và ở đây tên
      # cơ sở dữ liệu mặc định của hình ảnh PostgreSQL được sử dụng
...
Khi sử dụng [cài đặt nhiều cây](multi-tree.md), addon SharedPostgreSQL là một tùy chọn thuận tiện để lưu trữ tất cả các cây, bao gồm cả các cây mới được tạo qua API, trong một cơ sở dữ liệu PostgreSQL duy nhất mà không làm tổn hại đến quyền riêng tư hoặc bảo mật.
