...
- Ánh xạ các nhóm/nhóm OIDC sang các vai trò người dùng Gramps Web
...
!!! info
    Khi sử dụng biến môi trường, hãy nhớ thêm tiền tố `GRAMPSWEB_` vào mỗi tên thiết lập (ví dụ: `GRAMPSWEB_OIDC_ENABLED`). Xem [Tệp cấu hình so với biến môi trường](configuration.md#configuration-file-vs-environment-variables) để biết chi tiết.
...
`OIDC_ISSUER` | URL nhà phát hành của nhà cung cấp của bạn. Thông tin khám phá được lấy từ `<issuer>/.well-known/openid-configuration`.
...
`tree` là bắt buộc trong các cài đặt đa cây; nếu bỏ qua, hoặc truyền ID của một cây không tồn tại, việc đăng nhập sẽ thất bại. Trên một máy chủ đơn cây, `tree` là tùy chọn, nhưng nếu được cung cấp thì phải khớp với `TREE` đã được cấu hình.

Một danh tính OIDC được liên kết với chính xác một tài khoản Gramps Web, mà lại thuộc về chính xác một cây – việc đăng nhập vào một cây khác sẽ thất bại thay vì di chuyển tài khoản. Không có cách nào để liên kết một danh tính duy nhất tại nhà cung cấp với các tài khoản trong nhiều cây; người dùng cần truy cập vào nhiều cây cần có các danh tính riêng biệt tại nhà cung cấp (ví dụ: tên người dùng hoặc tài khoản khác nhau).

!!! warning
    Một tài khoản quản trị viên trang web không có cây liên kết (xem [tạo tài khoản quản trị](../administration/owner.md)) không thể đăng nhập qua OIDC, vì xác thực OIDC luôn yêu cầu một cây. Các tài khoản như vậy phải được tạo và xác thực bằng tên người dùng/mật khẩu cục bộ thay thế.
...
Trong đó `*` là một ký tự đại diện regex. Tùy thuộc vào trình phân tích cú pháp regex của nhà cung cấp của bạn, điều này cũng có thể là `.*` hoặc tương tự.
...
Gramps Web có thể tự động ánh xạ các nhóm hoặc vai trò OIDC từ nhà cung cấp danh tính của bạn sang các vai trò người dùng Gramps Web. Điều này cho phép bạn quản lý quyền người dùng một cách tập trung trong nhà cung cấp danh tính của bạn. Việc ánh xạ vai trò hoạt động giống nhau cho tất cả các nhà cung cấp, tích hợp hoặc tùy chỉnh.
...
Nếu không có thiết lập `OIDC_GROUP_*` nào được cấu hình, việc ánh xạ vai trò sẽ tắt và các vai trò sẽ được quản lý thủ công trong Gramps Web; các tài khoản OIDC mới sau đó sẽ được tạo ra ở trạng thái bị vô hiệu hóa và cần được phê duyệt bởi một chủ sở hữu hoặc quản trị viên hiện có (xem [Đăng nhập lần đầu và Khởi động](#first-login-and-bootstrapping) bên dưới).
...
- Nếu yêu cầu vai trò có mặt nhưng người dùng không thuộc về nhóm nào đã được ánh xạ, vai trò của họ sẽ được đặt thành bị vô hiệu hóa. Đây là một mặc định fail-closed, không phải là lỗi – Gramps Web không thể suy luận một vai trò cho một nhóm mà nó không nhận ra.
- Nếu yêu cầu vai trò hoàn toàn vắng mặt trong token, vai trò hiện có sẽ không thay đổi; một tài khoản mới vẫn mặc định ở trạng thái bị vô hiệu hóa.

!!! warning "Google không gửi yêu cầu nhóm"
    Các token của Google không bao giờ bao gồm yêu cầu `groups`, vì vậy với việc ánh xạ vai trò được kích hoạt, các đăng nhập Google rơi vào tình huống "yêu cầu vắng mặt" ở trên: người dùng hiện có giữ vai trò của họ, nhưng người dùng Google mới được tạo ra ở trạng thái bị vô hiệu hóa và cần phê duyệt thủ công. Hãy ghi nhớ điều này trước khi kích hoạt ánh xạ vai trò chỉ cho một nhà cung cấp khác – nó không, tự nó, vô hiệu hóa người dùng Google hiện có.

Microsoft Entra chỉ trả về vai trò ứng dụng và thành viên nhóm trong token ID, không từ điểm cuối thông tin người dùng. Gramps Web hợp nhất các yêu cầu của token ID vào phản hồi thông tin người dùng để `OIDC_ROLE_CLAIM` hoạt động giống như đối với các nhà cung cấp khác; nơi cả hai đều chứa một yêu cầu, giá trị thông tin người dùng sẽ được ưu tiên.

## Đăng nhập lần đầu và Khởi động

Các tài khoản mới được tạo thông qua OIDC bắt đầu ở trạng thái bị vô hiệu hóa trừ khi ánh xạ vai trò gán cho chúng một vai trò (xem ở trên). Trên một phiên bản hoàn toàn mới, không ai có thể phê duyệt một tài khoản bị vô hiệu hóa, và nếu `OIDC_DISABLE_LOCAL_AUTH` cũng được kích hoạt thì không có đăng nhập bằng mật khẩu nào để quay lại.

!!! warning "Cấu hình một nhóm chủ sở hữu/quản trị viên trước khi đăng nhập lần đầu"
    Trước khi bất kỳ ai đăng nhập qua OIDC lần đầu tiên, hãy thiết lập `OIDC_GROUP_OWNER` (hoặc `OIDC_GROUP_ADMIN`) và đảm bảo rằng người dùng đầu tiên thuộc về nhóm đó tại nhà cung cấp. Nếu không, phiên bản không thể được khởi động thông qua OIDC.
...
- Các nhà cung cấp tích hợp: `<provider>_<claim value>`, ví dụ: `microsoft_alice@contoso.com`
- Nhà cung cấp tùy chỉnh: giá trị yêu cầu thuần túy, ví dụ: `alice`

Một hậu tố số sẽ được thêm vào khi có sự trùng lặp. Không có cách nào để đổi tên người dùng của tài khoản được tạo ra qua OIDC sau đó; ngược lại, tên đầy đủ và địa chỉ email sẽ được làm mới trong mỗi lần đăng nhập.

Một đăng nhập OIDC không bao giờ gắn liền với một tài khoản cục bộ hiện có mà tình cờ chia sẻ địa chỉ email của nó – điều này là có chủ ý, vì việc liên kết các tài khoản bằng email là một vector chiếm đoạt tài khoản. Một người dùng đã có tài khoản cục bộ sẽ nhận được một tài khoản thứ hai, riêng biệt lần đầu tiên họ đăng nhập qua OIDC.
...
# Tùy chọn: Ánh xạ vai trò từ các nhóm OIDC sang vai trò Gramps
...
Hầu hết các cấu hình cho Keycloak có thể để ở mặc định của nó (*Client → Tạo client → Kích hoạt xác thực client*).
Có một vài ngoại lệ:
...
2. **Vai trò** – Vai trò có thể được gán ở cấp độ client hoặc toàn cầu theo realm.
    ...
    * Để làm cho các vai trò có thể nhìn thấy đối với Gramps, hãy điều hướng đến *Client Scopes* (phần cấp cao nhất, không nằm dưới client cụ thể), sau đó: *Roles → Mappers → vai trò client → Thêm vào thông tin người dùng → BẬT.*
