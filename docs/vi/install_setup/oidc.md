...
!!! info
    Khi sử dụng biến môi trường, hãy nhớ thêm tiền tố cho mỗi tên thiết lập với `GRAMPSWEB_` (ví dụ: `GRAMPSWEB_OIDC_ENABLED`). Xem [Tệp cấu hình vs. biến môi trường](configuration.md#configuration-file-vs-environment-variables) để biết thêm chi tiết.
...
Key | Mô tả
----|-------------
`OIDC_ENABLED` | Boolean, có bật xác thực OIDC hay không. Đặt thành `True`.
`OIDC_ISSUER` | URL phát hành của nhà cung cấp của bạn
...
1. **Đăng ký endpoint đăng xuất backchannel** trong cấu hình khách hàng của nhà cung cấp danh tính của bạn:
   ```
...
2. **Cấu hình nhà cung cấp của bạn** để gửi thông báo đăng xuất. Các bước cụ thể phụ thuộc vào nhà cung cấp của bạn:
   ...
   **Keycloak:**
   ...
   - Bật "Cần phiên đăng xuất Backchannel" nếu bạn muốn đăng xuất dựa trên phiên
...
!!! tip "Cách hoạt động"
    ...
    1. Nhà cung cấp gửi một `logout_token` JWT đến endpoint đăng xuất backchannel của Gramps Web
    ...
    4. Bất kỳ yêu cầu API mới nào với JWT của người dùng sẽ bị từ chối một khi các token hết hạn
...
Hầu hết các cấu hình cho Keycloak có thể để ở mặc định của nó (*Khách hàng → Tạo khách hàng → Bật xác thực khách hàng*).
Có một vài ngoại lệ:
...
2. **Vai trò** – Vai trò có thể được gán ở cấp khách hàng hoặc toàn cầu theo miền.
    ...
    * Để làm cho các vai trò hiển thị cho Gramps, hãy điều hướng đến *Phạm vi Khách hàng* (phần cấp cao nhất, không phải dưới khách hàng cụ thể), sau đó: *Vai trò → Bộ ánh xạ → vai trò khách hàng → Thêm vào thông tin người dùng → BẬT.*
